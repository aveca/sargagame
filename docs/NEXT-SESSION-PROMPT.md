...
8. **Le bug cache / Service Worker est RÉSOLU à l'origine** (commits `6cc567e` + `526efd2`). SW = `sargasses-v24` + force-reload onglets à l'activation, `sw.js`/`version.json` en `no-cache` via `.htaccess` (vérifié à l'origine LiteSpeed: OK). **NE RE-DÉBOGUE PAS le code.** Si tu déploies du code, bumpe `CACHE_NAME` dans `public/sw.js`.
   ⚠️ **MAIS MQ/GP sont derrière CLOUDFLARE** (les nouveaux domaines en direct cPanel). Cloudflare avait edge-cché l'ancien `sw.js` avec le header `immutable` (1 an) → `Cf-Cache-Status: HIT`, servait `v23` aux users malgré l'origine correcte. **ACTION SYSTÉMIQUE** : (a) purger le cache Cloudflare de `/sw.js` (one-shot pour débloquer l'existant), (b) ajouter une **Cloudflare Cache Rule "Bypass cache" sur `/sw.js` + `/version.json`**, (c) **automatiser une purge Cloudflare dans `daily-copernicus.yml` après le deploy FTP** (API `POST /zones/{zone}/purge_cache`, secrets `CF_API_TOKEN` + `CF_ZONE_ID` à créer). Sans ça, chaque deploy de `sw.js` met jusqu'à 1 an à atteindre les users MQ/GP. Diag: `curl -sI https://sargasses-martinique.com/sw.js` (edge) vs `--resolve sargasses-martinique.com:443:162.0.229.47` (origine).
