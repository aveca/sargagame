...
- **Live en prod aujourd'hui** (SW v175, MQ+GP) : Accueil A→Z (`home_az`), Plan-B fiche (`pw_planb`), badge H2S fiche (`pw_h2s`), fiabilité par régime, **carte `map_world`** (`WorldMapView` region-aware A/B 50/50), **fiche plongée `pw_beach_dive`** (`BeachDive.jsx` 6 beats + Veilleur v2 + scrub A/B 50/50).
- **En cours** : Phase B — demo-gate email (capture 0,35 %).
