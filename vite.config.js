  ...
  plugins: [
    ...
    <link rel="alternate" hreflang="es" href="https://${domain}/es/" />`
        ...
        const ldFaq = JSON.stringify({ '@context': 'https://schema.org', '@type': 'FAQPage', mainEntity: es ? [
          ...
          { '@type': 'Question', name: `¿Cuándo es la temporada de sargazo en ${name}?`, acceptedAnswer: { '@type': 'Answer', text: 'La llegada de sargazo suele alcanzar su pico de abril a septiembre, pero varía semana a semana con viento y corrientes. El monitoreo funciona todo el año con pronóstico de 7 días por playa.' } },
          ...
          { '@type': 'Question', name: `Y a-t-il des sargasses en ${name} en ce moment ?`, acceptedAnswer: { '@type': 'Answer', text: `Ouvrez la carte en direct pour voir l'état du jour de chaque plage de ${name}. Chacune des ${beaches.length} plages suivies reçoit un Beach Score 0-100 qui combine sargasses, houle, vent, température de l'eau et soleil — mis à jour 4 fois par jour avec les données satellite.` } },
          { '@type': 'Question', name: `Quelles plages de ${name} sont sans sargasses aujourd'hui ?`, acceptedAnswer: { '@type': 'Answer', text: `Les conditions changent plage par plage : ${communes.join(', ')} ne sont pas touchées de la même façon le même jour. La carte classe toutes les plages quotidiennement pour choisir la plus propre — généralement les côtes abritées et sous le vent.` } },
          { '@type': 'Question', name: `Quand a lieu la saison des sargasses en ${name} ?`, acceptedAnswer: { '@type': 'Answer', text: `L'arrivée des sargasses culmine généralement d'avril à septembre, mais varie d'une semaine à l'autre selon le vent et les courants. Le suivi fonctionne toute l'année avec des prévisions à 7 jours par plage.` } },
          { '@type': 'Question', name: 'Comment les données sargasses sont-elles mesurées ?', acceptedAnswer: { '@type': 'Answer', text: `La détection utilise l'indice satellite AFAI (Alternative Floating Algae Index) échantillonné au large de chaque plage, combiné au vent et aux courants dans une prévision à 7 jours par plage. Données actualisées plusieurs fois par jour.` } },
        ] : [
          { '@type': 'Question', name: `Is there sargassum or seaweed in ${name} right now?`, acceptedAnswer: { '@type': 'Answer', text: `Open the live map to see today's status for every ${name} beach. Each of the ${beaches.length} monitored beaches gets a Beach Score 0-100 combining sargassum, seaweed, swell, wind, water temperature and sun — updated 4× daily from NOAA satellite data.` } },
          ...
          { '@type': 'Question', name: `When is sargassum season in ${name}?`, acceptedAnswer: { '@type': 'Answer', text: 'Sargassum and seaweed influx usually peaks between April and September, but landings vary week to week with currents and wind. Monitoring runs year-round with a 7-day forecast for every beach.' } },
        ...
        const ldOrg = JSON.stringify({ '@context': 'https://schema.org', '@type': 'Organization', name: siteName, url: `https://${domain}`, logo: `https://${domain}/icon-512.png`, description: es ? `Estado diario del sargazo y Beach Score 0-100 para las playas de ${name}` : fr ? `État quotidien des sargasses et Beach Score 0-100 pour les plages de ${name}` : `Daily sargassum status and Beach Score 0-100 for ${name} beaches`, areaServed: [name, REGION.country].filter(Boolean), knowsAbout: [es ? 'sargazo' : fr ? 'sargasses' : 'sargassum', es ? 'sargazo y algas marinas' : fr ? 'sargasses et algues' : 'sargassum and seaweed', es ? 'playas' : fr ? 'plages' : 'beaches', name, REGION.country].filter(Boolean) })
        const ldLocalBiz = JSON.stringify({ '@context': 'https://schema.org', '@type': 'LocalBusiness', name: `${siteName} — Beach Conditions Monitoring`, url: `https://${domain}/`, description: es ? `Monitoreo en vivo de sargazo y algas marinas playa por playa en ${name}. Mapa satelital, Beach Score 0-100 y pronóstico 7 días.` : fr ? `Suivi en direct des sargasses plage par plage en ${name}. Carte satellite, Beach Score 0-100 et prévisions 7 jours.` : `Live sargassum and seaweed monitoring per beach in ${name}. Satellite map, Beach Score 0-100 and 7-day forecast.`, areaServed: { '@type': 'Place', name: name, address: REGION.country ? { '@type': 'PostalAddress', addressCountry: REGION.countryCode || '' } : undefined }, serviceType: es ? 'Monitoreo de Condiciones de Playa' : fr ? 'Suivi des conditions de plage' : 'Beach Conditions Monitoring', provider: { '@type': 'Organization', name: siteName } })
        html = html
        ...
        const byCommune = communes.map(c => `<h2>${c} — ${es ? 'playas monitoreadas a diario' : fr ? 'plages surveillées quotidiennement' : 'beaches monitored daily'}</h2>\n        <p>${beaches.filter(b => b.commune === c).map(b => b.name).join(', ')}.</p>`).join('\n        ')
        const displayName = (REGION.seo && REGION.seo.homeName) || name
        const noscriptRegion = es ? `<noscript>
        <h1>Sargazo y Algas en ${displayName} Hoy — estado en vivo de cada playa (2026)</h1>
        <p>¿Qué playa de ${displayName} está sin sargazo hoy? Estado diario de ${beaches.length} playas (${beachNames.slice(0, 6).join(', ')}…) con datos satelitales de la NOAA, Beach Score 0-100 y pronóstico de 7 días por playa.</p>
        ${byCommune}
        <h2>Temporada de sargazo en ${displayName} ${today.slice(0, 4)}</h2>
      ...
      </noscript>` : fr ? `<noscript>
        <h1>Sargasses ${displayName} en temps réel — carte et plages aujourd'hui (2026)</h1>
        <p>Carte des sargasses en ${displayName} en temps réel. Où se baigner aujourd'hui ? État de ${beaches.length} plages mis à jour quotidiennement grâce aux données satellite. Prévisions sargasses 7 jours et alertes push gratuites.</p>
        ${byCommune}
        <h2>Saison des sargasses en ${displayName} ${today.slice(0, 4)}</h2>
        <p>L'arrivée des sargasses culmine généralement d'avril à septembre, mais varie d'une semaine à l'autre selon le vent et les courants. La carte est mise à jour 4 fois par jour, toute l'année, pour choisir la bonne plage avant de partir.</p>
      </noscript>` : `<noscript>
        <h1>${name} Sargassum & Seaweed Today — live status for every beach (2026)</h1>
        <p>Which ${name} beach is free of sargassum and seaweed today? Daily status for ${beaches.length} beaches (${beachNames.slice(0, 6).join(', ')}…) from NOAA satellite data, with a Beach Score 0-100 and a 7-day forecast per beach.</p>
        ${byCommune}
        <h2>Sargassum & seaweed season in ${name} ${today.slice(0, 4)}</h2>
        <p>Sargassum and seaweed arrivals usually peak from April to September, but vary week to week with wind and currents. The map is updated 4× daily, year-round, so you can pick the right beach before you go.</p>
      </noscript>`
    ...
    // SEO : après build, copier index vers /carte-sargasses/, /previsions/, /alertes/ et /en/ avec titres/meta dédiés
    {
      ...
      closeBundle() {
        ...
        if (IS_NEW_REGION) {
          ...
          // P0 acquisition — page « aujourd'hui » /today/ (EN) /hoy/ (ES),
          // data-driven quotidienne (sitemap.xml régional patché sur disque).
          try {
            const { generateTodayPages } = _require('./scripts/lib/today-pages.cjs')
            generateTodayPages(REGION, resolve(__dirname, 'dist'))
          } catch (e) { console.warn('   ⚠ page aujourd\u2019hui région:', e.message) }
          return
        }
        ...
        try {
          ...
          const pages = [
            { path: 'carte-sargasses', enPath: 'en/sargassum-map', esPath: 'es/mapa-sargazo', title: 'Carte des sargasses Martinique aujourd\'hui — temps réel (2026)', desc: 'Carte interactive des sargasses en Martinique aujourd\'hui : quelles plages sont propres, lesquelles éviter, état en direct par satellite Copernicus. Mise à jour quotidienne.' },
            ...
            // Cluster Communes (Guadeloupe) — hub-and-spoke vers les fiches plage GP (GP_ONLY dans prepare-ftp)
            { path: 'sargasses-deshaies', enPath: null, title: 'Sargasses à Deshaies (Guadeloupe) — Grande Anse, La Perle', desc: 'Y a-t-il des sargasses à Deshaies ? Sur la côte sous-le-vent de Basse-Terre, La Grande Anse, la Plage de la Perle et la plage du bourg comptent parmi les mieux protégées de l\'archipel. État du jour, carte en temps réel.' },
            { path: 'sargasses-sainte-anne-guadeloupe', enPath: null, title: 'Sargasses à Sainte-Anne (Guadeloupe) — Bois Jolan, Caravelle', desc: 'Y a-t-il des sargasses à Sainte-Anne en Guadeloupe ? État du jour de Bois Jolan, la Caravelle, la plage du bourg et Anse Champagne, sur le sud de Grande-Terre. Carte en temps réel, prévision 7 jours.' },
            { path: 'sargasses-gosier', enPath: null, title: 'Sargasses au Gosier (Guadeloupe) — état des plages aujourd\'hui', desc: 'Y a-t-il des sargasses au Gosier ? État du jour de la Datcha, la plage du bourg, Petit Havre et Saint-Félix, au sud de Grande-Terre. Carte en temps réel, prévision 7 jours, alternatives propres.' },
            { path: 'sargasses-saint-francois', enPath: null, title: 'Sargasses à Saint-François (Guadeloupe) — Raisins Clairs, Pointe des Châteaux', desc: 'Y a-t-il des sargasses à Saint-François ? État du jour des Raisins Clairs, de la Pointe des Châteaux et d\'Anse Tarare, à la pointe sud-est de Grande-Terre. Carte en temps réel, prévision 7 jours.' },
            { path: 'sargasses-bouillante', enPath: null, title: 'Sargasses à Bouillante (Guadeloupe) — Malendure, Îlets Pigeon et plages du jour', desc: 'Y a-t-il des sargasses à Bouillante ? Sur la côte sous-le-vent de Basse-Terre, Malendure et la Réserve Cousteau comptent parmi les secteurs les mieux protégés de l\'archipel. État du jour, carte en temps réel, prévision 7 jours.' },
            { path: 'sargasses-les-saintes', enPath: null, title: 'Sargasses aux Saintes (Terre-de-Haut) — Pompierre, Pain de Sucre et plages du jour', desc: 'Y a-t-il des sargasses aux Saintes ? Sur l\'archipel de Terre-de-Haut, baies abritées et anses sauvages cohabitent. État du jour de Pompierre, Pain de Sucre et Anse Crawen, carte en temps réel, prévision 7 jours.' },
            { path: 'sargasses-marie-galante', enPath: null, title: 'Sargasses à Marie-Galante (Guadeloupe) — Capesterre, Anse Canot et plages du jour', desc: 'Y a-t-il des sargasses à Marie-Galante ? La côte est (Capesterre) est très exposée, la côte ouest (Saint-Louis, Anse Canot) plus abritée. État du jour, carte en temps réel, prévision 7 jours.' },
            // Cluster Topic (Guadeloupe) — synonyme « algues » = sargasses (gap GSC 250 imp)
            { path: 'algues-guadeloupe', enPath: null, title: "Algues sur les plages de Guadeloupe : ce sont les sargasses — carte du jour", desc: "Ces algues brunes qui s'échouent sur les plages de Guadeloupe sont les sargasses. D'où viennent-elles, sont-elles dangereuses, quelles plages sont épargnées aujourd'hui ? Carte en temps réel et prévision 7 jours." },
          ...
          // Pages communes FR générées (long-tail "sargasses <commune>", data-backed) :
          // 32 communes littorales sans page dédiée, chacune listant ses vraies plages
          // (→ /plages/<slug>/ où l'état live est affiché). Poussées AVANT la boucle
          // d'écriture pour être générées, maillées et miroitées comme les pages à la main.
          const _commune = _require('./scripts/lib/commune-seo-pages.cjs')
          pages.push(..._commune.communeSeoList())
          const COMMUNE_SLUGS = _commune.communeSlugsByIsland()
          ...
          const editorialContent = {
            ...
            'sargasses-deshaies': `<article><h1>Sargasses à Deshaies (Guadeloupe) — Grande Anse, La Perle et plages du jour</h1><p>Y a-t-il des <strong>sargasses à Deshaies</strong> aujourd'hui ? Au nord-ouest de Basse-Terre, Deshaies est sur la <strong>côte sous-le-vent</strong>, à l'abri du relief : c'est l'un des secteurs les <em>mieux</em> protégés des sargasses de tout l'archipel, épargné des alizés porteurs d'algues. Les épisodes y sont rares — mais en année record, vérifiez l'<a href="/carte-sargasses/">état du jour sur la carte</a> avant de partir.</p><h2>Les plages de Deshaies</h2><ul><li><a href="/plages/la-grande-anse-deshaies/">La Grande Anse</a> — la plus longue plage de Basse-Terre, sable doré bordé de cocotiers (décor de « Meurtres au paradis »). Presque toujours épargnée.</li><li><a href="/plages/plage-de-la-perle/">Plage de la Perle</a> — baie en fer à cheval abritée, eaux calmes, snorkeling.</li><li><a href="/plages/plage-de-deshaies/">Plage de Deshaies</a> (bourg) — petite plage du village, accès facile, restaurants à deux pas.</li></ul><h2>Pourquoi Deshaies est-elle si protégée ?</h2><p>La façade ouest de Basse-Terre est mise à l'abri du flux atlantique par le massif de la Soufrière. C'est la même logique qui protège <a href="/plages/plage-de-malendure/">Malendure</a> plus au sud. En pleine <a href="/saison-sargasses-guadeloupe/">saison de pic</a> (mai à septembre), quand Grande-Terre est saturée, Deshaies reste presque toujours praticable — un refuge fiable.</p><h2>Et si un épisode rare survenait ?</h2><p>Dans les années record, des bancs peuvent atteindre temporairement la côte sous-le-vent. Dans ce cas, consultez les <a href="/meilleures-plages-guadeloupe-sargasses/">plages les plus propres du jour</a> et la <a href="/previsions/">prévision 7 jours</a> pour ajuster.</p><h2>Être prévenu pour Deshaies</h2><p><a href="/alertes/">Activez une alerte gratuite</a> sur la Grande Anse ou la Plage de la Perle — utile surtout en pic de saison.</p><p><a href="/carte-sargasses/">Carte en temps réel</a> · <a href="/previsions/">Prévisions 7 jours</a> · <a href="/saison-sargasses-guadeloupe/">Saison des sargasses GP</a> · <a href="/sargasses-gosier/">Sargasses au Gosier</a></p></article>`,
            'sargasses-sainte-anne-guadeloupe': `<article><h1>Sargasses à Sainte-Anne (Guadeloupe) — Bois Jolan, Caravelle et plages du jour</h1><p>Y a-t-il des <strong>sargasses à Sainte-Anne</strong> en Guadeloupe ? Sur la côte sud de Grande-Terre, Sainte-Anne est une station balnéaire prisée pour ses lagons turquoise — mais sa position l'expose aux flux atlantiques quand les bancs remontent. Lagons protégés d'un côté, plages plus ouvertes de l'autre : vérifiez l'<a href="/carte-sargasses/">état du jour plage par plage</a> avant de venir. <em>(Sainte-Anne existe aussi en Martinique — cette page concerne la Guadeloupe.)</em></p><h2>Les plages de Sainte-Anne (Guadeloupe)</h2><ul><li><a href="/plages/plage-bois-jolan/">Plage de Bois Jolan</a> — lagon peu profond protégé par la barrière de corail, idéal en famille.</li><li><a href="/plages/plage-de-la-caravelle/">Plage de la Caravelle</a> — la plus connue (Club Med), sable blanc et lagon abrité.</li><li><a href="/plages/plage-de-sainte-anne/">Plage de Sainte-Anne</a> (bourg) — animée, accessible, marché et commerces à deux pas.</li><li><a href="/plages/anse-champagne/">Anse Champagne</a> — voisine de la Caravelle, familiale et calme.</li></ul><h2>Quand les sargasses touchent-elles Sainte-Anne ?</h2><p>La <a href="/saison-sargasses-guadeloupe/">saison court d'avril à octobre</a>, pic de mai à août. Sur le sud de Grande-Terre, les arrivages sont intermittents : un lagon protégé peut rester propre alors qu'une plage ouverte, à quelques kilomètres, reçoit un banc. D'où l'intérêt de comparer les plages de la commune sur la <a href="/carte-sargasses/">carte</a>.</p><h2>Où aller si Sainte-Anne est touchée ?</h2><p>Cap vers la côte sous-le-vent de Basse-Terre, le refuge le plus fiable : <a href="/plages/la-grande-anse-deshaies/">La Grande Anse de Deshaies</a> ou <a href="/plages/plage-de-malendure/">Malendure</a>. Ou consultez les <a href="/meilleures-plages-guadeloupe-sargasses/">meilleures plages propres du jour</a>.</p><h2>Être prévenu pour Sainte-Anne</h2><p><a href="/alertes/">Activez une alerte gratuite</a> sur Bois Jolan ou la Caravelle et recevez l'état du jour avant de partir.</p><p><a href="/carte-sargasses/">Carte en temps réel</a> · <a href="/previsions/">Prévisions 7 jours</a> · <a href="/sargasses-gosier/">Sargasses au Gosier</a> · <a href="/sargasses-saint-francois/">Sargasses à Saint-François</a></p></article>`,
            'sargasses-gosier': `<article><h1>Sargasses au Gosier (Guadeloupe) — état des plages aujourd'hui</h1><p>Y a-t-il des <strong>sargasses au Gosier</strong> ? Cœur touristique du sud de Grande-Terre, face à l'îlet du Gosier, la commune aligne hôtels et plages urbaines très fréquentées. Sa position sud l'expose aux flux atlantiques par épisodes : l'état change d'une plage à l'autre et d'une semaine à l'autre. Vérifiez l'<a href="/carte-sargasses/">état du jour</a> avant de poser votre serviette.</p><h2>Les plages du Gosier</h2><ul><li><a href="/plages/la-datcha/">Plage de la Datcha</a> — la plus connue, sable blanc, bars et restaurants, navette pour l'îlet.</li><li><a href="/plages/plage-du-gosier/">Plage du Gosier</a> (bourg) — face à l'îlet, animée, accessible.</li><li><a href="/plages/petit-havre-gosier/">Petit Havre</a> — crique plus sauvage à l'est, état plus variable.</li><li><a href="/plages/saint-felix/">Plage de Saint-Félix</a> — secteur résidentiel, plus calme.</li></ul><h2>Quand les sargasses arrivent-elles au Gosier ?</h2><p>La <a href="/saison-sargasses-guadeloupe/">saison s'étend d'avril à octobre</a>, pic de mai à août. Le sud de Grande-Terre encaisse les flux atlantiques : comparez les plages de la commune sur la <a href="/carte-sargasses/">carte en temps réel</a> — la Datcha peut être propre quand Petit Havre reçoit un banc.</p><h2>Où aller si Le Gosier est touché ?</h2><p>Direction la côte sous-le-vent de Basse-Terre : <a href="/plages/la-grande-anse-deshaies/">La Grande Anse de Deshaies</a>, <a href="/plages/plage-de-malendure/">Malendure</a>, ou les <a href="/meilleures-plages-guadeloupe-sargasses/">plages les plus propres du jour</a>.</p><h2>Être prévenu pour Le Gosier</h2><p><a href="/alertes/">Activez une alerte gratuite</a> sur la Datcha ou Petit Havre : l'état du jour vous parvient avant de partir.</p><p><a href="/carte-sargasses/">Carte en temps réel</a> · <a href="/previsions/">Prévisions 7 jours</a> · <a href="/sargasses-sainte-anne-guadeloupe/">Sargasses à Sainte-Anne</a> · <a href="/sargasses-deshaies/">Sargasses à Deshaies</a></p></article>`,
            'sargasses-saint-francois': `<article><h1>Sargasses à Saint-François (Guadeloupe) — Raisins Clairs, Pointe des Châteaux</h1><p>Y a-t-il des <strong>sargasses à Saint-François</strong> ? À la pointe sud-est de Grande-Terre, Saint-François combine un lagon abrité très prisé et des plages sauvages exposées plein est. C'est ce contraste qui rend la vérification utile : le lagon peut être impeccable quand la côte au vent reçoit des bancs. Consultez l'<a href="/carte-sargasses/">état du jour</a> avant de venir.</p><h2>Les plages de Saint-François</h2><ul><li><a href="/plages/plage-des-raisins-clairs/">Plage des Raisins Clairs</a> — grand lagon peu profond protégé par la barrière, familial, le plus sûr de la commune.</li><li><a href="/plages/pointe-des-chateaux/">Pointe des Châteaux</a> — site sauvage classé, plages exposées plein est : superbe mais aux premières loges des arrivages.</li><li><a href="/plages/anse-tarare/">Anse Tarare</a> — crique naturiste près de la Pointe des Châteaux, état variable.</li></ul><h2>Quand les sargasses touchent-elles Saint-François ?</h2><p>La <a href="/saison-sargasses-guadeloupe/">saison court d'avril à octobre</a>, pic de mai à août. La côte est de Grande-Terre est en première ligne : la Pointe des Châteaux reçoit souvent les premiers bancs, tandis que le lagon des Raisins Clairs reste plus protégé. Comparez sur la <a href="/carte-sargasses/">carte en temps réel</a>.</p><h2>Où aller si Saint-François est touché ?</h2><p>Cap vers la côte sous-le-vent de Basse-Terre : <a href="/plages/la-grande-anse-deshaies/">La Grande Anse de Deshaies</a>, <a href="/plages/plage-de-malendure/">Malendure</a>, ou les <a href="/meilleures-plages-guadeloupe-sargasses/">meilleures plages propres du jour</a>.</p><h2>Être prévenu pour Saint-François</h2><p><a href="/alertes/">Activez une alerte gratuite</a> sur les Raisins Clairs et recevez l'état du jour avant de partir.</p><p><a href="/carte-sargasses/">Carte en temps réel</a> · <a href="/previsions/">Prévisions 7 jours</a> · <a href="/sargasses-sainte-anne-guadeloupe/">Sargasses à Sainte-Anne</a> · <a href="/sargasses-gosier/">Sargasses au Gosier</a></p></article>`,
            'sargasses-bouillante': `<article><h1>Sargasses à Bouillante (Guadeloupe) — Malendure, Îlets Pigeon et plages du jour</h1><p>Y a-t-il des <strong>sargasses à Bouillante</strong> aujourd'hui ? Sur la <strong>côte sous-le-vent</strong> de Basse-Terre, Bouillante est mise à l'abri du flux atlantique par le massif de la Soufrière : c'est l'un des secteurs les <em>mieux</em> protégés des sargasses de tout l'archipel, et le point de départ de la célèbre Réserve Cousteau. Les épisodes y sont rares — mais en année record, vérifiez l'<a href="/carte-sargasses/">état du jour sur la carte</a> avant de partir plonger ou poser votre serviette.</p><h2>Les plages de Bouillante</h2><ul><li><a href="/plages/plage-de-malendure/">Plage de Malendure</a> — sable volcanique gris, face aux Îlets Pigeon : le spot de snorkeling et de plongée n°1 de Guadeloupe (Réserve Cousteau). Presque toujours épargnée.</li><li><a href="/plages/anse-de-pigeon/">Anse à Pigeon</a> — embarcadère vers les îlets, eaux calmes et claires.</li><li><a href="/plages/petite-plage-malendure/">Petite Plage de Malendure</a> — crique voisine plus tranquille, idéale en famille.</li><li><a href="/plages/plage-des-amandiers/">Plage des Amandiers</a> et <a href="/plages/anse-a-sable/">Anse à Sable</a> — petites plages ombragées du secteur.</li></ul><h2>Pourquoi Bouillante est-elle si protégée ?</h2><p>La façade ouest de Basse-Terre est abritée du flux atlantique porteur d'algues par le relief volcanique. C'est la même logique qui protège <a href="/plages/la-grande-anse-deshaies/">Deshaies</a> plus au nord. En pleine <a href="/saison-sargasses-guadeloupe/">saison de pic</a> (mai à septembre), quand le sud de Grande-Terre est saturé, Bouillante reste presque toujours praticable — un refuge fiable pour la baignade et la plongée.</p><h2>Et si un épisode rare survenait ?</h2><p>Dans les années record, des bancs peuvent atteindre temporairement la côte sous-le-vent. Dans ce cas, consultez les <a href="/meilleures-plages-guadeloupe-sargasses/">plages les plus propres du jour</a> et la <a href="/previsions/">prévision 7 jours</a> pour ajuster votre sortie. Le secteur garde presque toujours une crique praticable, mais mieux vaut le confirmer avant de prendre la route littorale jusqu'à la Réserve Cousteau.</p><h2>Être prévenu pour Bouillante</h2><p><a href="/alertes/">Activez une alerte gratuite</a> sur Malendure et recevez l'état du jour avant de partir — utile surtout en pic de saison.</p><p><a href="/carte-sargasses/">Carte en temps réel</a> · <a href="/previsions/">Prévisions 7 jours</a> · <a href="/sargasses-deshaies/">Sargasses à Deshaies</a> · <a href="/sargasses-gosier/">Sargasses au Gosier</a></p></article>`,
            'sargasses-les-saintes': `<article><h1>Sargasses aux Saintes (Terre-de-Haut) — Pompierre, Pain de Sucre et plages du jour</h1><p>Y a-t-il des <strong>sargasses aux Saintes</strong> aujourd'hui ? Petit archipel au sud de Basse-Terre, l'Îlet de <strong>Terre-de-Haut</strong> aligne baies abritées et anses sauvages : son relief crée un patchwork où une plage protégée peut être impeccable quand une anse ouverte, à dix minutes de marche, reçoit un banc. C'est ce contraste qui rend la vérification utile avant la traversée en navette. Consultez l'<a href="/carte-sargasses/">état du jour sur la carte</a> avant de réserver votre bateau.</p><h2>Les plages de Terre-de-Haut</h2><ul><li><a href="/plages/plage-de-pompierre/">Plage de Pompierre</a> — la plus célèbre, longue baie en croissant protégée par deux îlots, eau calme et turquoise. Souvent abritée.</li><li><a href="/plages/plage-pain-de-sucre/">Plage du Pain de Sucre</a> — crique de carte postale au pied du piton, snorkeling, accès par un sentier.</li><li><a href="/plages/anse-crawen/">Anse Crawen</a> — plage naturiste isolée à l'ouest, plus sauvage.</li><li><a href="/plages/anse-rodrigue/">Anse Rodrigue</a> et <a href="/plages/anse-du-figuier-saintes/">Anse du Figuier</a> — petites plages tranquilles du secteur sud.</li></ul><h2>Quand les sargasses touchent-elles Les Saintes ?</h2><p>La <a href="/saison-sargasses-guadeloupe/">saison court d'avril à octobre</a>, pic de mai à août. L'archipel est globalement mieux protégé que la côte atlantique de Grande-Terre, mais les anses orientées au vent peuvent recevoir des bancs par épisodes. Comparez les plages avant de monter dans la navette : un aller-retour à l'aveugle coûte une demi-journée et le prix d'un billet de bateau. En pratique, l'archipel garde presque toujours une baie abritée fréquentable le jour même, ce qui en fait une valeur sûre quand Grande-Terre est saturée.</p><h2>Où aller si une anse est touchée ?</h2><p>Sur l'îlet même, basculez vers une baie abritée (Pompierre, Pain de Sucre). Si l'épisode est large, la côte sous-le-vent de Basse-Terre — <a href="/sargasses-deshaies/">Deshaies</a>, Bouillante — reste le refuge le plus fiable. Repérez les <a href="/meilleures-plages-guadeloupe-sargasses/">plages les plus propres du jour</a>.</p><h2>Être prévenu pour Les Saintes</h2><p><a href="/alertes/">Activez une alerte gratuite</a> sur Pompierre et recevez l'état du jour avant d'embarquer.</p><p><a href="/carte-sargasses/">Carte en temps réel</a> · <a href="/previsions/">Prévisions 7 jours</a> · <a href="/sargasses-deshaies/">Sargasses à Deshaies</a> · <a href="/sargasses-sainte-anne-guadeloupe/">Sargasses à Sainte-Anne</a></p></article>`,
            'sargasses-marie-galante': `<article><h1>Sargasses à Marie-Galante (Guadeloupe) — Capesterre, Anse Canot et plages du jour</h1><p>Y a-t-il des <strong>sargasses à Marie-Galante</strong> aujourd'hui ? La « Grande Galette » est une île ronde au sud-est de la Guadeloupe, en plein dans le flux atlantique : sa <strong>côte est et sud-est</strong> — Capesterre-de-Marie-Galante en tête — compte parmi les secteurs les <em>plus exposés</em> de tout l'archipel, tandis que la <strong>côte ouest</strong> (Saint-Louis, Grand-Bourg) est nettement plus abritée. D'une plage à l'autre, l'état peut donc être radicalement différent le même jour. Avant de prendre le ferry, vérifiez l'<a href="/carte-sargasses/">état du jour sur la carte</a>.</p><h2>Les plages de Marie-Galante</h2><ul><li><a href="/plages/plage-de-capesterre-mg/">Plage de Capesterre</a> (Capesterre-de-Marie-Galante) — la célèbre plage de la Feuillère, sable blanc et cocotiers, mais orientée plein est : aux premières loges des arrivages en saison.</li><li><a href="/plages/anse-canot-marie-galante/">Anse Canot</a> (Saint-Louis) — sur la côte ouest abritée, lagon turquoise et calme, l'une des valeurs les plus sûres de l'île.</li><li><a href="/plages/plage-de-moustique/">Plage de Moustique</a> et <a href="/plages/anse-de-vieux-fort-mg/">Anse de Vieux-Fort</a> (Saint-Louis) — criques sauvages de la côte ouest, souvent épargnées.</li><li><a href="/plages/plage-de-grand-bourg/">Plage de Grand-Bourg</a> — au sud-ouest, près du bourg principal et de l'embarcadère, plus accessible.</li><li><a href="/plages/anse-piton/">Anse Piton</a> (Saint-Louis) — petite anse discrète du nord-ouest.</li></ul><h2>Quand les sargasses touchent-elles Marie-Galante ?</h2><p>La <a href="/saison-sargasses-guadeloupe/">saison court d'avril à octobre</a>, pic de mai à août. Île basse exposée plein Atlantique, Marie-Galante encaisse les flux directement sur sa façade est et sud-est : Capesterre reçoit souvent les bancs en premier, alors que la côte ouest (Saint-Louis) reste plus protégée. C'est tout l'intérêt de comparer les plages sur la <a href="/carte-sargasses/">carte en temps réel</a> avant d'embarquer : un aller-retour en ferry coûte une demi-journée et un billet, autant viser directement le bon côté de l'île.</p><h2>Où aller si une plage de Marie-Galante est touchée ?</h2><p>Sur l'île même, basculez vers la côte ouest abritée (Anse Canot, Moustique, Vieux-Fort). Si l'épisode est large, la côte sous-le-vent de Basse-Terre — <a href="/sargasses-deshaies/">Deshaies</a>, Bouillante — reste le refuge le plus fiable de l'archipel. Repérez les <a href="/meilleures-plages-guadeloupe-sargasses/">plages les plus propres du jour</a> et consultez la <a href="/previsions/">prévision 7 jours</a> pour choisir votre créneau.</p><h2>Être prévenu pour Marie-Galante</h2><p><a href="/alertes/">Activez une alerte gratuite</a> sur Anse Canot ou Capesterre et recevez l'état du jour avant de prendre le bateau — utile surtout en pic de saison.</p><p><a href="/carte-sargasses/">Carte en temps réel</a> · <a href="/previsions/">Prévisions 7 jours</a> · <a href="/sargasses-deshaies/">Sargasses à Deshaies</a> · <a href="/meilleures-plages-guadeloupe-sargasses/">Meilleures plages GP</a></p></article>`,
            'algues-guadeloupe': "<article><h1>Algues sur les plages de Guadeloupe : ce sont les sargasses — carte du jour</h1><p>Vous cherchez où en sont les <strong>algues en Guadeloupe</strong> ? Ces grandes nappes brunes qui s'échouent sur le sable et dégagent une odeur d'œuf pourri en séchant sont les <strong>sargasses</strong> — une algue brune pélagique qui dérive depuis l'Atlantique. « Algues » et « sargasses » désignent ici la même chose. Avant de choisir votre plage, consultez l'<a href=\"/carte-sargasses/\">état du jour sur la carte en temps réel</a>.</p><h2>Algues ou sargasses : la même chose</h2><p>En Guadeloupe, on parle couramment d'« algues » pour les <strong>sargasses</strong>. Fraîches et en mer, elles sont brun-doré et inoffensives ; c'est en s'accumulant sur la plage puis en se décomposant qu'elles posent problème. À ne pas confondre avec les rares « algues rouges » ou les herbiers marins : les nappes qui recouvrent le littoral antillais par bancs, d'avril à octobre, sont bien les sargasses.</p><h2>D'où viennent ces algues ?</h2><p>Elles proviennent de la grande ceinture de sargasses de l'Atlantique, entre l'Afrique et le Brésil, et remontent vers les Antilles portées par les courants et les alizés. La <a href=\"/saison-sargasses-guadeloupe/\">saison court d'avril à octobre</a>, avec un pic de mai à septembre. La côte atlantique (est de Grande-Terre, Capesterre) est la plus exposée ; la côte sous-le-vent de Basse-Terre est la mieux protégée.</p><h2>Ces algues sont-elles dangereuses ?</h2><p>En mer, non. Sur la plage, leur décomposition dégage du sulfure d'hydrogène (H₂S) et de l'ammoniac, qui peuvent irriter yeux et voies respiratoires près des gros dépôts. Détails et précautions sur notre page <a href=\"/sargasses-sante-symptomes/\">sargasses et santé</a>. La règle simple : éviter de stationner au contact d'algues en train de sécher.</p><h2>Quelles plages de Guadeloupe sont sans algues aujourd'hui ?</h2><p>Les bancs bougent vite : une plage couverte un jour peut être dégagée deux jours plus tard. Repérez les <a href=\"/meilleures-plages-guadeloupe-sargasses/\">plages les plus propres du jour</a>, recalculées à partir des données satellite. Les valeurs sûres restent la côte sous-le-vent : <a href=\"/plages/la-grande-anse-deshaies/\">Grande Anse de Deshaies</a>, la <a href=\"/plages/plage-de-la-perle/\">Plage de la Perle</a> et <a href=\"/plages/plage-de-malendure/\">Malendure</a> à Bouillante.</p><p><a href=\"/carte-sargasses/\">Carte en temps réel</a> · <a href=\"/previsions/\">Prévisions 7 jours</a> · <a href=\"/alertes/\">Activer une alerte gratuite</a> · <a href=\"/sargasses-deshaies/\">Sargasses à Deshaies</a> · <a href=\"/sargasses-gosier/\">Sargasses au Gosier</a></p></article>",
            'sargasses-sante-symptomes': `<article><h1>Sargasses et santé : symptômes, qui est à risque, que faire</h1><p>Les sargasses sont-elles dangereuses pour la santé ? <strong>Fraîches et en mer, non.</strong> Le risque apparaît lorsqu'elles <strong>s'accumulent et se décomposent</strong> sur la plage : elles dégagent alors deux gaz, le sulfure d'hydrogène (H₂S, odeur d'œuf pourri) et l'ammoniac (NH₃). Aux Antilles, en Martinique et Guadeloupe, c'est cette décomposition — pas l'algue elle-même — qui justifie les précautions. Pour les seuils techniques, voir notre page <a href="/danger-sargasses-h2s/">dangers du H₂S</a>.</p><h2>Quels symptômes ?</h2><p>À faible concentration, l'exposition au H₂S et à l'ammoniac peut provoquer&nbsp;:</p><ul><li>Irritation des yeux (picotements, larmoiement) et de la gorge</li><li>Maux de tête, nausées, vertiges</li><li>Gêne respiratoire, toux — surtout chez les personnes sensibles</li><li>Mauvais goût métallique, odeur d'œuf pourri tenace</li></ul><p>Ces symptômes sont généralement <strong>réversibles</strong> : ils régressent en s'éloignant de la zone et en respirant un air propre. L'odeur peut aussi noircir les bijoux en argent et les appareils électroniques — signe qu'il vaut mieux ne pas s'attarder.</p><h2>Qui doit être particulièrement prudent ?</h2><ul><li><strong>Femmes enceintes</strong> — par précaution, éviter les zones d'échouage en décomposition.</li><li><strong>Nourrissons et jeunes enfants</strong> — plus sensibles, plus près du sol où le gaz stagne.</li><li><strong>Personnes asthmatiques ou avec une maladie respiratoire/cardiaque</strong>.</li><li><strong>Personnes âgées</strong>.</li></ul><p>Ces publics devraient éviter de séjourner près d'amas en décomposition, surtout par vent faible où le gaz se concentre.</p><h2>Que faire concrètement ?</h2><ol><li><strong>Choisir une plage propre.</strong> Le moyen le plus simple d'éviter tout risque : vérifier l'<a href="/carte-sargasses/">état du jour</a> et viser les <a href="/plages-sans-sargasses/">plages sans sargasses</a>.</li><li><strong>S'éloigner si ça sent l'œuf pourri.</strong> L'odeur est le signal d'alerte — ne pas rester sous le vent d'un amas.</li><li><strong>Ne pas laisser les enfants jouer dans les algues échouées.</strong></li><li><strong>Anticiper.</strong> <a href="/alertes/">Les alertes gratuites</a> préviennent quand votre plage change d'état.</li></ol><p>En cas de symptômes persistants (gêne respiratoire qui ne passe pas), consultez un professionnel de santé. Les autorités (ARS, préfectures) publient des seuils d'évacuation pour les riverains les plus exposés.</p><h2>Sources officielles</h2><p>Pour les recommandations sanitaires détaillées et les seuils officiels d'exposition au H₂S, consultez les autorités de santé : <a href="https://www.martinique.ars.sante.fr/les-algues-sargasses-un-phenomene-sanitaire" target="_blank" rel="noopener">ARS Martinique — les algues sargasses</a>, <a href="https://www.guadeloupe.ars.sante.fr/sargasses" target="_blank" rel="noopener">ARS Guadeloupe — informations sargasses</a> et <a href="https://www.santepubliquefrance.fr/regions/antilles/donnees/sargasses" target="_blank" rel="noopener">Santé publique France — Antilles</a>.</p><p><a href="/danger-sargasses-h2s/">Dangers du H₂S en détail</a> · <a href="/nettoyer-sargasses/">Que faire face à un échouage</a> · <a href="/carte-sargasses/">Carte en temps réel</a> · <a href="/alertes/">Alertes gratuites</a></p></article>`,
            ...
            'plages-sans-sargasses': (() => {
              ...
              try {
                ...
                return `<article><h1>Plages sans sargasses en Martinique et Guadeloupe</h1>` +
                  ...
                  `Consultez aussi les <a href="/previsions/">prévisions 7 jours par plage</a>, la page <a href="/aujourdhui/">où se baigner aujourd'hui</a> et activez les <a href="/alertes/">alertes push gratuites</a>.</p></article>`
              } catch (_e) {
          ...
          const faqSchemas = {
            ...
            'sargasses-bouillante': [
              { q: "Y a-t-il des sargasses à Malendure (Bouillante) ?", a: "Très rarement. Malendure est sur la côte sous-le-vent de Basse-Terre, protégée du flux atlantique par le massif de la Soufrière : c'est l'un des secteurs les mieux épargnés de Guadeloupe, ce qui en fait un spot de plongée fiable même en saison de pic. Vérifiez l'état du jour sur la carte avant de partir." },
              { q: "Peut-on plonger à la Réserve Cousteau malgré les sargasses ?", a: "Oui, dans la quasi-totalité des cas : les Îlets Pigeon et la Réserve Cousteau, au large de Malendure, sont protégés et restent praticables même quand le sud de Grande-Terre est touché. En année record, consultez la prévision 7 jours pour confirmer." },
              { q: "Où aller près de Bouillante si une plage est touchée ?", a: "Restez sur la côte sous-le-vent : Deshaies (Grande Anse, La Perle) au nord est le refuge le plus fiable. Consultez aussi les meilleures plages propres du jour sur la carte." },
            ],
            'sargasses-les-saintes': [
              { q: "Y a-t-il des sargasses à Pompierre aux Saintes ?", a: "Rarement : la plage de Pompierre est une baie en croissant protégée par deux îlots, ce qui l'abrite bien des arrivages. Les anses ouvertes au vent de Terre-de-Haut peuvent être plus exposées le même jour. Vérifiez l'état du jour sur la carte avant la traversée." },
              { q: "Faut-il vérifier les sargasses avant d'aller aux Saintes en navette ?", a: "Oui, ça vaut le coup : un aller-retour en bateau coûte une demi-journée et un billet. Comparer les plages de Terre-de-Haut sur la carte avant d'embarquer évite de découvrir un banc sur place — d'autant que l'archipel reste globalement protégé, donc une bonne plage est presque toujours disponible." },
              { q: "Les Saintes sont-elles épargnées par les sargasses ?", a: "Globalement mieux protégées que la côte atlantique de Grande-Terre grâce à leur position et à leurs baies abritées, mais pas garanties : les anses orientées au vent reçoivent des bancs par épisodes en pleine saison. La carte du jour fait la différence plage par plage." },
            ],
            'sargasses-marie-galante': [
              { q: "Y a-t-il des sargasses à Marie-Galante en ce moment ?", a: "Cela dépend du côté de l'île. La côte est et sud-est (Capesterre-de-Marie-Galante) est très exposée aux flux atlantiques et reçoit souvent les bancs en premier, tandis que la côte ouest (Saint-Louis, Anse Canot, Grand-Bourg) est plus abritée. Vérifiez l'état du jour plage par plage sur la carte avant de prendre le ferry." },
              { q: "Quelle plage de Marie-Galante est la plus épargnée par les sargasses ?", a: "La côte ouest est la plus sûre : Anse Canot, Plage de Moustique et Anse de Vieux-Fort (commune de Saint-Louis) sont protégées des arrivages atlantiques et restent souvent praticables même quand Capesterre est touchée." },
              { q: "Faut-il vérifier les sargasses avant de prendre le ferry pour Marie-Galante ?", a: "Oui, ça vaut le coup : la traversée coûte une demi-journée et un billet. Comme l'île a un côté très exposé (est) et un côté abrité (ouest), comparer les plages sur la carte avant d'embarquer évite de découvrir un banc sur place et permet de viser directement le bon littoral." },
            ],
            ...
            'algues-guadeloupe': [
              { q: "Les algues sur les plages de Guadeloupe, ce sont les sargasses ?", a: "Oui. Les grandes nappes brunes qui s'échouent sur le littoral guadeloupéen d'avril à octobre sont les sargasses, une algue pélagique venue de l'Atlantique. « Algues » et « sargasses » désignent ici la même chose. À ne pas confondre avec les herbiers ou les rares algues rouges." },
              { q: "Les algues (sargasses) de Guadeloupe sont-elles dangereuses ?", a: "En mer, non. Le risque vient de leur décomposition sur la plage, qui dégage du sulfure d'hydrogène (odeur d'œuf pourri) et de l'ammoniac, pouvant irriter yeux et voies respiratoires près des gros dépôts. Il suffit d'éviter de stationner au contact d'algues en train de sécher." },
              { q: "Quelles plages de Guadeloupe sont sans algues aujourd'hui ?", a: "La côte sous-le-vent de Basse-Terre (Grande Anse de Deshaies, la Perle, Malendure) est la plus souvent épargnée. Comme les bancs bougent vite, consultez la carte du jour et la liste des plages les plus propres, recalculées à partir des données satellite." },
            ],
            'faq': [
          ...
          const PUBLISHED_BY_SLUG = {
            ...
            'sargasses-bouillante': '2026-06-23',
            'sargasses-les-saintes': '2026-07-01',
            'sargasses-marie-galante': '2026-07-02',
            'algues-guadeloupe': '2026-07-03',
          ...
          Object.assign(PUBLISHED_BY_SLUG, _commune.communeDates())
          const pubDateFor = (slug) => PUBLISHED_BY_SLUG[slug] || '2026-04-01'
          // MedicalWebPage : signal YMYL pour la page santé (sujet médical, type
          // dédié + lastReviewed honnête). PAS de reviewedBy : aucun relecteur
          // humain nommé → un reviewedBy:Organization qui « relit » sa propre
          // page serait du schema theater (risque d'action manuelle). Additif au
          // schema Article/FAQ existant, jamais en remplacement.
          const HEALTH_SLUGS = new Set(['sargasses-sante-symptomes', 'en/sargassum-health'])
          const medicalWebPageTag = (slug, url, lang) => {
            if (!HEALTH_SLUGS.has(slug)) return ''
            const o = { '@context': 'https://schema.org', '@type': 'MedicalWebPage', url, inLanguage: lang, lastReviewed: pubDateFor(slug), audience: { '@type': 'MedicalAudience', audienceType: 'Patient' }, about: { '@type': 'MedicalCondition', name: lang === 'en' ? 'Hydrogen sulfide (H2S) and ammonia exposure from decomposing sargassum' : "Exposition au sulfure d'hydrogène (H₂S) et à l'ammoniac des sargasses en décomposition" } }
            return `\n    <script type="application/ld+json">\n    ${JSON.stringify(o)}\n    </script>`
          }
          for (const { path: p, title, desc, enPath, esPath } of pages) {
            ...
            if (editorialContent[p]) {
              ...
              const medicalSchemaTag = medicalWebPageTag(p, `https://sargasses-martinique.com/${p}/`, 'fr')
              pageHtml = pageHtml
                .replace('</head>', `${ogArticleTags}\n    <script type="application/ld+json">\n    ${articleSchema}\n    </script>${faqSchemaTag}${medicalSchemaTag}\n</head>`)
          ...
          for (const ep of enPages) {
            ...
            if (editorialContent[ep.path]) {
              ...
              const medicalSchemaTag = medicalWebPageTag(ep.path, `https://sargasses-martinique.com/${ep.path}/`, 'en')
              epOut = epOut
                .replace('</head>', `${ogArticleTags}\n    <script type="application/ld+json">\n    ${articleSchema}\n    </script>${faqSchemaTag}${medicalSchemaTag}\n</head>`)
          ...
          const STABLE_EDITORIAL = '2026-06-21'
...
<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
  ...
  <url><loc>${d}/pro/en/sargassum-forecast-cancun-hotels/</loc><lastmod>${today}</lastmod><changefreq>monthly</changefreq><priority>0.6</priority></url>
  <url><loc>${d}/pro/en/sargassum-forecast-punta-cana-hotels/</loc><lastmod>${today}</lastmod><changefreq>monthly</changefreq><priority>0.6</priority></url>
  <url><loc>${d}/pro/en/sargassum-forecast-miami-hotels/</loc><lastmod>${today}</lastmod><changefreq>monthly</changefreq><priority>0.6</priority></url>
  ...
  <url><loc>${d}/pro/es/pronostico-sargazo-cancun-hoteles/</loc><lastmod>${today}</lastmod><changefreq>monthly</changefreq><priority>0.6</priority></url>
  <url><loc>${d}/pro/es/pronostico-sargazo-punta-cana-hoteles/</loc><lastmod>${today}</lastmod><changefreq>monthly</changefreq><priority>0.6</priority></url>
...
${isGP ? `  <url><loc>${d}/bulletin-sargasses-guadeloupe/</loc><lastmod>${today}</lastmod><changefreq>daily</changefreq><priority>0.9</priority></url>
  ...
  <url><loc>${d}/sargasses-bouillante/</loc><lastmod>${today}</lastmod><changefreq>weekly</changefreq><priority>0.7</priority></url>
   <url><loc>${d}/sargasses-les-saintes/</loc><lastmod>${today}</lastmod><changefreq>weekly</changefreq><priority>0.7</priority></url>
  <url><loc>${d}/sargasses-marie-galante/</loc><lastmod>${today}</lastmod><changefreq>weekly</changefreq><priority>0.7</priority></url>
  <url><loc>${d}/algues-guadeloupe/</loc><lastmod>${today}</lastmod><changefreq>weekly</changefreq><priority>0.7</priority></url>
  ...
  <url><loc>${d}/cgv.html</loc><lastmod>${today}</lastmod><changefreq>yearly</changefreq><priority>0.3</priority></url>
  <url><loc>${d}/remboursement.html</loc><lastmod>${today}</lastmod><changefreq>yearly</changefreq><priority>0.3</priority></url>
</urlset>
`
...
console.log('   → BreadcrumbList ajouté à /carte-sargasses/, /previsions/ et /alertes/')
          ...
          // Pages communes générées → sitemaps (MQ communes dans MQ, GP dans GP) :
          // sans ça submit-indexnow/Google ne les découvrent pas (elles ne sont
          // listées que dans le template hardcodé pour les communes écrites à la main).
          for (const s of COMMUNE_SLUGS.mq) sitemapMQBeaches += `  <url><loc>https://sargasses-martinique.com/${s}/</loc><lastmod>${today}</lastmod><changefreq>weekly</changefreq><priority>0.6</priority></url>\n`
          for (const s of COMMUNE_SLUGS.gp) sitemapGPBeaches += `  <url><loc>https://sargasses-guadeloupe.com/${s}/</loc><lastmod>${today}</lastmod><changefreq>weekly</changefreq><priority>0.6</priority></url>\n`
          ...
          // P0 acquisition — pages « aujourd'hui » /aujourdhui/ (MQ canonical +
          // miroir _gp/, sitemaps -martinique/-guadeloupe patchés sur disque).
          // Contenu 100 % sargassum.json du jour, URL stable, lastmod quotidien.
          try {
            const { generateTodayPages } = _require('./scripts/lib/today-pages.cjs')
            generateTodayPages(null, outDir)
          } catch (e) { console.warn('   ⚠ pages aujourd\u2019hui:', e.message) }

          // P1 acquisition/communauté — /presse/ (kit data-driven) + /soutenir/
          // (Veilleur, sans reçu fiscal). Core MQ/GP uniquement (miroir _gp/).
          try {
            const { generateCivicPages } = _require('./scripts/lib/civic-pages.cjs')
            generateCivicPages(null, outDir)
          } catch (e) { console.warn('   ⚠ pages civiques:', e.message) }
