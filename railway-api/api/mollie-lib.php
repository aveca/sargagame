...
function getMollieClient(): SgMollieClient {
    // Load from env vars (Render) with local file fallback
...
function mol_b2c_pass_grant(string $paymentId, string $pass, string $email, array $metadata = []): array {
    ...
    error_log("[mol_b2c_pass_grant] pass=$pass paymentId=$paymentId days=$days session_id=" . ($sessionId ?? 'NULL') . " expires=" . date('c', $expiresAt) . " mirror_ok=" . ($mirrorOk ? 'true' : 'false'));
