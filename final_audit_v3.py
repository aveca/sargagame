...
with sync_playwright() as pw:
    ...
    # 2. Click a beach label (sg-maplabel)
    beach_labels = page.locator('.sg-maplabel:visible')
    log(f'Visible sg-maplabel: {beach_labels.count()}')
    
    beach_clicked = False
    for i in range(min(beach_labels.count(), 3)):
        label = beach_labels.nth(i)
        if label.is_visible():
            txt = label.inner_text().strip()[:50]
