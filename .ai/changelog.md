## 2026-09-20 — K3 MAP DECLUTTER MERGÉ + DÉPLOYÉ + QA PROD OK (PR #692, commit `2070589a`)

**Agent** : release/qa-agent. **Séquence** : #691 squash-mergée (`f271417c`, A13 J+1 + LeadCapture z-index, CI 7/7) → #692 rebasée `--onto` (commit `c5f6de7` exclu, seul `0efaac7`→`6c92e54` conservé, conflits `.ai/*` résolus par union) → CI #692 7/7 SUCCESS (code-scan AI = échec infra `CAPIError 400 model not supported`, jamais d'analyse du diff, advisory) → squash merge #692 (`2070589a`) → Deploy Live SUCCESS (build + Workers + Pages 6/6 + purge + health 6/6).
**QA prod mobile 390×844 (sargasses-martinique.com)** : `/` = carte épurée (héros 0, B2B 0, email 0, digest 0, recherche 1, labels 10/3, près-de-moi 1, BottomNav 5, center-hit = `.sg-maplabel`, 0 console error) ; `/?mapdeclutter=0` = chrome historique restauré (héros + Voir→ + B2B). Screenshots `Temp/opencode/k3-prod-{default,rollback}.png`. Verdict : la Carte respire, l'Accueil porte la décision.
