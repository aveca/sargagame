## 2026-09-20 — A13 J+1 PORT DÉPLOYÉ + QA PROD OK (PR #694 mergée, main `f0314464`)

**Agent** : release/qa-agent. **Deploy Live run 35483343174** : build + Workers 3/3 + Pages 6/6 + purge OK, health 5/6 (sargassummiami.com = 1 échec TRANSITOIRE : curl exit 35 TLS côté runner, 0 tentative HTTP ; Miami répond 200/app en direct après — infra, pas le code A13 ; à re-jouer au besoin). **QA prod mobile 390×844 (sargasses-martinique.com, pin réel → ChasseDetail)** : défaut J+1 = `s-ok now` + badge INCLUS, tap → 0 paywall + fiche intacte, J+2 teaser, 0 console error ; `?j1_free=0` → J+1 teaser + cadenas, badge absent. Screenshots `Temp/opencode/k3-proda13-{default,rollback}.png`. Verdict : A13 LIVE VALIDÉ sur le chemin pin.
