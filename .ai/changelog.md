...
**Pourquoi** : avant, un pass one-time PayPal ou un état abo n'avait aucun reflet serveur (fichiers JSON AppScript-Side only) → perte de vérité en cas de rotation/perte. Désormais mirror Supabase additivement (jamais de régression du chemin natif vérif-live).
**Preuves** : paypal-grants-mirror ALL PASS (22 audits + 16/16 harness PHP réel) · php -l ×3 OK · build exit 0 · bundle 38,2 Ko ≤ 210 · smoke 4/4 + `SMOKE_GATE=PASS` · Playwright funnel-payment 13/13 · 0 appel réseau/écriture prod pendant les tests.
**Rollback** : revert du commit (le mirror est purement additif ; couper le mirror laisse le flux natif intact).
...
## 2026-09-20 — B2B SALES ENGINE PHASE 2 : importeur SIRENE MQ + scoring déterministe (branche `agent/data/b2b-phase2-sirene`, PR DRAFT — NON MERGÉE)

**Agent** : data-agent (Phase 2). **Périmètre** : additif pur — `scripts/lib/sirene-client.cjs` (client SIRENE v3.11 : Luhn SIREN/SIRET + exception La Poste documentée, A→active / F→inactive / sinon unknown, normalisation vers les colonnes Phase 1, fetch injecté, lat/lon JAMAIS inventés), `scripts/lib/b2b-store.cjs` (upsert read-then-write siren/siret — PostgREST ne sait pas upserter nos index uniques partiels ; dry = zéro HTTP, fail-closed sans credentials, `probeSchema()` refuse `--execute` tant que le schéma n'est pas appliqué), `scripts/automation/sirene-mq-import.cjs` (Martinique 972 uniquement, DRY_RUN défaut, `--from-file`, `--execute` fail-closed, logs synthétiques zéro PII, aucun contact/email/téléphone généré), `scripts/lib/b2b-scoring.cjs` (pur : 6 composantes 25/25/20/15/10/5 → 0-100, seuils configurables 85/70/50/30, `reasons[]`, `model_version=deterministic-v1`, inactives capées à 25). **Schéma** (prod vide, migration sûre) : `prospect_scores` passe du modèle PFCV hérité aux 6 composantes déterministes ; `companies`/`company_establishments` += colonnes SIRENE natives (ape_label, employee_range, is_micro_enterprise, creation_date, siege, source_updated_at). `.env.example` documente SIRENE_CONSUMER_KEY/SECRET. **AUCUNE exécution réelle, aucun workflow touché, aucun envoi/appel/paiement.**
**Preuves** : b2b-scoring 42/42 · sirene-import 81/81 (fixtures locales, mock PostgREST mémoire, idempotence rejouée prouvée, `--execute` sans config → exit 1) · b2b-phase2-guards 33/33 (anti-activation : pas de réf outreach/email/paiement/vocal, aucun workflow ne lance l'importeur, aucune clé en dur) · b2b-sales-engine-schema 95/95 · b2b-phase2-runbook (cohérence doc↔code, compteurs réels vérifiés). (Compteurs corrigés au réel mesuré : total 251.)
**BLOCKER inchangé (séparé du code)** : tables NON créées en prod tant que `SUPABASE_ACCESS_TOKEN` n'est pas régénéré (401 sur apply-supabase-schema.yml). Activation réelle = attendre GREEN + validation fondateur.
**Rollback** : branche non mergée → aucun rollback nécessaire.
...
**Preuves** : passoffer-cta-copy 9/9 · passoffer-paths 11/11 · probe Playwright réel mobile : défaut = « Voir la prévision 7 jours → », rollback `?sgcta=0` = « Voir mes plages propres → » (via funnel réel — le deep-link `?paywall=1` nettoie la query, limite documentée partagée par tous les flags), clic CTA → checkout, 0 pageerror · build exit 0 · bundle 38,1 Ko ≤ 210 · smoke 4/4 · Playwright funnel-payment 13/13 + j0-sprint 7/7.
**Rollback** : `?sgcta=0` ou revert du commit.

## 2026-09-19 — K3 MAP DECLUTTER / EXPLORATION PURE (branche `agent/coding/k3-map-declutter`)

**Agent** : coding-agent (scope audit K3, défaut ON, rollback `?mapdeclutter=0`). **Cause** : la vue Carte cumulait réponse + exploration (~25 interactifs, 7 niveaux hiérarchie) avec un doublon décisionnel (héros carte vs HomeDashboard XP). **Fix (rendu conditionnel seul, 0 logique supprimée)** : `mapDeclutterOff` (WorldMapView.jsx, pattern flags existants) garde 5 panneaux — héros « Où te baigner » (`?maphero=0` préservé), carte Ma Plage inline (`?mapmy=0` préservé), sticker email, chip B2B légende (`?promap=0` préservé), digest « Cette semaine ». Conservés : RegionNav, fraîcheur, recherche, canvas/pins/labels/readiness, légende, « près de moi », scrub J0→J5, BottomNav. Zéro paiement/z-index/tracking/fiche touché.
**Preuves** : build 0 (389 modules) · bundle 38.1 ≤ 210 · smoke 4/4 · map-declutter 17/17 (NOUVEAU) · funnel+bottomnav 21/21 · ma-plage 8/8 (basculée `?mapdeclutter=0` = couverture rollback) · DOM 390px : 48→43 interactifs primo-visite (−5 mesuré, −8+ à chrome plein), héros/B2B 1→0 défaut et 1→1 rollback, labels 10/3 identiques, center-hit = `.sg-maplabel` (canvas atteignable), 0 console error · screenshots `Temp/opencode/k3-declutter-{on,off}.png`.
**Fichiers** : `src/WorldMapView.jsx` (1 const + 5 gardes + commentaire), `tests/e2e/ma-plage.spec.ts` (8 goto rollback + note), `scripts/tests/map-declutter.test.cjs` (NOUVEAU).

## 2026-09-18 — SHIP G1 : lead capture → Supabase primaire, Apps Script en backup (branche `agent/coding/g1-supabase-leads`)

**Agent** : coding-agent (release post-#687, worktree isolé `sargagame-g1`). **Périmètre verrouillé** : 5 fichiers, 0 autre — `src/supabasePhotos.js` (+66 additif : `submitLeadToSupabase`, `buildB2CLeadRow`, `leadDomain`, `leadRegionCode` — miroir exact du vocabulaire LeadCapture, rollback `?lead_sb=0`), `src/Sargasses_PROD.jsx` (UNIQUEMENT le hunk G1 : import `submitLeadToSupabase` + réécriture `submitLead` = primaire Supabase + backup Apps Script inchangé + `sg_email_submit` réparé — l'ancien code ne l'émettait JAMAIS (early-return beacon + `island` hors scope = ReferenceError avalé) ; hunks A13 volontairement EXCLUS), `src/LeadCapture.jsx` (POST direct REST `/rest/v1/{b2c_alerts|b2b_leads}` à la clé anon au lieu du hop worker), `scripts/tests/lead-capture-g1.test.cjs` + `scripts/lib/supabase-leads.test.cjs` (contrats). **Rollback** : `?lead_sb=0` + revert du commit.
**Diagnostic prod 2026-09-18 (prouvé live, zéro écriture)** : `POST https://sargasses-martinique.com/api/supabase {table,insert}` → **404 {"error":"Not found"}** (route zone non câblée, catch-all Pages) → la bannière LeadCapture **perdait 100 % des leads en prod avec un faux succès affiché** (« Alertes activées ! » dans le catch) — panne silencieuse, pas de fallback. Probe RLS (`POST .../rest/v1/b2c_alerts {}` et `b2b_leads {}` avec clé anon) → **400 23502 not-null email** = la policy insert anon passe (aucune ligne écrite). Aucune migration SQL requise (schéma existant, tables + policies déjà en place).
**Preuves** : lead-capture-g1 8/8 · supabase-leads 21/21 · paywall-email-pre + social-proof ALL PASS (A1 #686 intact) · probe Playwright réel mobile 390×844 sur build local : bannière → saisie → Activer → **1 seul POST `.../rest/v1/b2c_alerts`** (apikey anon, row canonique `{email, region, domain, beaches:[], status:'active'}`, message succès affiché, **zéro appel à `/api/supabase`**, réseau intercepté → 0 écriture prod) · build exit 0 · bundle 38,1 Ko ≤ 210 · smoke 4/4 · Playwright funnel-payment 13/13 + j0-sprint 7/7.
**Détail honnêteté** : double-écriture Supabase + Apps Script dans `submitLead` = dual-write **intentionnel** (backup, lecture Sheet inchangée), documenté ; analytics `sg_email_submit`/`sg_lead_b2c_submit` inchangés ; aucune PII supplémentaire. Divergence mineure vs la version #685 : 1 ligne de commentaire corrigée (« worker /api/supabase » → « écriture directe REST ») pour refléter le fix routage.
## 2026-09-18 — F1 STALE CONTRAST (branche `agent/ui/f1-stale-contrast`, scope F1 uniquement)

**Agent** : ui-agent (decision scope F1, F2 backlog / F3 documente / F4 clos). **Cause** : WorldMapView.jsx:1915 branche stale `#B87A00` sur pastille (peinte blanche, pire-cas source creme) -> CR 3.34 < AA 4.5. **Fix (1 valeur)** : `#8a5a00` (encre moderate deja en palette) -> CR 5.49 creme / 5.93 blanc ; branche fresh `#00786C` (4.99) intacte ; ni layout, ni copy, ni comportement.
**Preuves** : f1-stale-contrast 6/6, build 0, bundle 38.1, smoke 4/4 exit 0, E2E bottomnav+funnel 21/21, computed navigateur 5.93 (etait 3.61) mobile+desktop, esbuild OK, diff --check OK.
**Fichiers** : `src/WorldMapView.jsx` (1 ligne + commentaire), `scripts/tests/f1-stale-contrast.test.cjs` (NOUVEAU).

## 2026-09-18 — SHIP partenaires→WhatsApp support (scope #685 extrait, branche `agent/coding/partners-whatsapp-ship`)

**Agent** : coding-agent (release post-#686). **Périmètre verrouillé** : 7 fichiers, 0 autre — `regions/mq.json` (MQ transport local = Sargagame Support WhatsApp wa.me/596596106124, `isWhatsApp:true`, shopping Lovelly retiré), `scripts/automation/gen-context-partners.cjs` (champ `isWhatsApp` propagé), `src/lib/partners.js` (`partnerOutboundUrl` : `?text=` prérempli contextuel pour WhatsApp, aucun prix/commission inventé), `src/components/PartnerContext.jsx` (carte support dédiée : glyph WhatsApp, badge « Support », partenaire nommé `Sargagame Support · Écris-nous sur WhatsApp`, rel sans `sponsored`, kind tracké `support`), `src/lib/partners-catalog.json` (RÉGÉNÉRÉ par le générateur depuis regions — source de vérité respectée), `scripts/tests/partners-contract.test.cjs` (contrat MAJ), `tests/e2e/partners-context.spec.ts` (E2E MAJ). **Rollback** : remettre l'ancien bloc partners dans regions/mq.json + regen, ou `?partnerctx=0` (kill-switch existant).
**Preuves** : contrat partners 37/37 · `assertAllRegionsValid` OK · suite npm test 173/175 (2 échecs = worktrees jolly-yalow préexistants gitignorés, hors repo) · build exit 0 · bundle 38,1 Ko ≤ 210 · smoke 4 tokens PASS · Playwright : funnel-payment 13/13, j0-sprint 7/7, partners-context 2/2 isolé + 10/10 avec ma-plage ; 1 échec observé en parallélisme lourd (timing overlays cookie+bannière email, bug produit CONNU UX-QA-002/003 déjà en MASTER_AUDIT — non causé par ce scope, non modifié).
...
**Preuves** : j0 7/7 · ma-plage 8/8 · weekhub 5/5 · partners 2/2 · e9 5/5 (mobile+desktop) · funnel 13/13 · around-me 10/10 · bottomnav 8/8 · p1-03 week-hub 11/11 · smoke 4 tokens+SMOKE_GATE=PASS · bundle 38.1 Ko ≤ 210 · unitaires paywall-email-pre/social-proof/lead-capture-g1/passoffer-paths ALL PASS. Labels desktop 1440 : visibles ~200 ms, 5 arbitrés.
**⚠️ Concurrence inter-itérations** : une session parallèle a commit `67b495ab1` (PremiumModal seul, état hybride avec `community: __COMM` orphelin de `readCommOverride()`) et rétabli les autres fichiers à l'état antérieur ; le fix-QA a été ré-appliqué intégralement au-dessus. TOUT reste à committer (arbitrage merge = fondateur).
...
**Preuves** : build exit 0 (5.45s) · bundle 38.1 Ko ≤ 210 · esbuild OK · Playwright réel (vite preview local) mobile 390×844 + desktop 1440×900 : fiche plage ouverte → CTA « Débloquer les prévisions 7 jours » → modal z1260 > fiche z1200, `elementFromPoint` centre = panel (plus la fiche), CTA « Voir mes plages propres » pointable, Fermer cliquable avec fermeture réelle, fiche intacte après retour. Avant-fix confirmé occlus 4× (runs `.ai/ux-agent/runs/`).

## 2026-09-16 — CONTINUOUS UX EXPLORER : prototype bouclé (exploration → findings → skill → replay OK) + 1 BLOCKER UX découvert

**Agent** : ux-agent (mission UX/browser, branche `agent/ux/continuous-explorer`). **Boucle démontrée** : PROD → exploration LLM (Webwright `local_browser`, navigateur **Chromium local**, LLM distant = NVIDIA `moonshotai/kimi-k3` via backend `openrouter` pointé sur NIM — json_schema strict validé) → screenshots → findings → **skill `inspect-sargagame-home`** (`.ai/ux-agent/skills/`) → **replay déterministe Playwright OK** (mobile + desktop, exit 0, sans reconstruction manuelle).
**Findings (honnêtes, preuve à l'appui)** : UX-M-001 email capture recouvre bouton cookies « Refuser » ; UX-M-002 × Fermer du modal premium intercepté par la vague SVG de la fiche ; UX-D-001 nav « ◉ Carte » bloquée par la capture email ; UX-D-002 « Plus tard » inopérant sur modal premium desktop ; 400 `/api/mollie.php` + 404 `/api/b2b-partners.json` (URLs identifiées par le replay).
**BLOCKER découvert par le replay (UX-R2-003)** : clic « Débloquer les prévisions 7 jours » depuis la fiche plage → modal « Prévisions premium » s'ouvre en DOM **SOUS** la fiche (z 1100 < 1200, probe elementFromPoint = `.lc-detail-body`) → offre invisible, paywall muet pour l'utilisateur. Confirmé mobile+desktop, 4 tentatives.
**Fichiers** : `.ai/ux-agent/{webwright-run.cjs, run-explore.cjs, normalize-run.cjs, task-observe.txt, .gitignore, webwright-config/*}`, `.ai/ux-agent/skills/inspect-sargagame-home/{skill.json, run.py}`, runs sur disque (`.ai/ux-agent/runs/*`, gitignorés). **0 produit touché** (observe-only).
**NEXT** : arbitrer UX-R2-003 (fix z-index/fermeture fiche = money-path direct) → si décidé, task coding séparée ; brancher le replay en gate CI UX.
...
**Fix produit (`src/WorldMapView.jsx` seul)** : `DISMISS_BTN_STYLE` 44×44 grammaire comic + `data-testid="sg-hero-dismiss"` (id + aria-label conservés) + repli stateful `heroFolded` (init `sg_hero_fold`, garde du bloc, `?maphero=0` intact). AroundMeController, Mollie/KV/B2B, partners, data : NON touchés, 0 donnée inventée.
**Harnais (assertions intactes, mêmepattern)** : `selectors` en ARG d'`evaluate` (bottomnav ×2, j0 `openFirstBeach`) · `openPaywallViaNav` clique l'onglet premium par texte (nth(2)=Carte en 5 onglets) · repli héros porté dans j0 · pont comic→data en boucle ~15 s · dismissal wall « N PLAGES » partagé + re-vérifié avant vote/rapport · vote en match exact · contrat j0-sprint aligné sur `selectors.*` (valeur `[data-sg-labels-ready]` vérifiée).
**Preuves** : build 385 exit 0 · bundle 38,1 Ko ≤ 210 · smoke 4/4 (`map+fiche+paywall`, `ERRORS=[]`, ghost `[]`, `RM_INFINITE=[]`) · funnel-payment 13/13 · bottomnav 8/8 · j0-sprint 7/7 ×2 (14/14) · responsive 3/3 · `npm test` 117/119 (2 échecs = filets `.claude/worktrees/jolly-yalow` préexistants, hors repo) · sonde : boundary absente, ready `10/3`, labels 10/3, × présent+testid, tap→fiche OK, 0 pageerror · php -l N/A (0 .php).
**Fichiers (produit)** : `src/WorldMapView.jsx` (+16/-4). **Harnais** : `tests/e2e/bottomnav-redesign.spec.ts`, `tests/e2e/j0-sprint.spec.ts`, `scripts/tests/j0-sprint-contract.test.cjs`.
**NEXT** : merger ce fix → CI verte → merger #672 puis rebaser/merger #678 → deploy-live → health 6/6.
...
**Nouvelle IA** : BottomNav 5 onglets Accueil / Plages / Carte / Ma Plage / Pass (isPremium : Pass masqué, inchangé) · `src/components/ExperienceReset.jsx` (NOUVEAU, chunk lazy 6,8 Ko gzip, 0 octet eager) : HomeDashboard (situation du jour, meilleur choix, recherche, à explorer, suivis, teaser Pass), PlagesExplorer (recherche + filtres statut/tri/activités/favoris + 20 cartes riches score/alternative/fav/comparer), CompareSheet (dialogue 1-3 plages, meilleur choix, ouvrir/suivre), SuiviDashboard (multi-favoris + alertes + vues récentes via sg_last_beaches + Pass) · historique local sg_last_beaches alimenté dans onBeachClick · 6 events (nav_tab, compare_add/open, plages_filter, home_best_open, suivi_alert_toggle) dans SG_FUNNEL_EVENTS + FUNNEL_KEYS (2 scripts) · overlays en position:fixed (doctrine #root effondré 19px : absolute = clics morts, prouvé hit-test BODY→BUTTON) + shell padding-top 204px sous header/RegionNav + contraste clair-sur-navy.
**Rollback** : `?newia=0` = BottomNav 3 onglets historique, prouvé (nav Carte/Plages/Premium, xp-home absent).
**Fix collatéral build-bloquant** : `src/components/MaPlageView.jsx` cassée par session concurrente non committée (slot transport dupliqué, `</div>` manquants → vite build rouge) → réparée à intention égale (1 slot transportUrl + tracking outbound conservés) ; money-path concurrent (mollie-lib.php USD, espace/index.html) NON touché, NON committé.
**Gates** : build 385 OK · bundle 37.9 Ko ≤ 210 · smoke 4/4 (map+fiche+paywall, ERRORS=[], ghost=[], RM_INFINITE=[]) · E2E funnel 12/13 (1 échec `.sg-maplabel` data-dépendant : satellite 2 j stale → hero persistant ; prouvé NON-régression : échoue à l'identique avec `?newia=0` + serveur preview frais) · audit mobile 360/390/430 : 5 onglets visibles, 0 overflow, 0 tap <44px, 0 pageerror, 10 labels carte, compare ouvert, fiche ouverte (2602+ chars) · 0 .php touché par moi · money-path intact.
**Fichiers** : src/Sargasses_PROD.jsx, src/components/ExperienceReset.jsx (NOUVEAU), src/components/MaPlageView.jsx (réparation JSX), scripts/automation/funnel-from-supabase.cjs, scripts/automation/daily-stats-check.cjs.
