...
- **pw_calm promu défaut** (`638371ab`) : copy positive saison-calme à 85% (15% holdout). Plus de golden-hour gaté aux minorités.
- **REFONTE paywall = continuation de scène** (`7ce45477`, sign-off fondateur GO + pulse #1 modal→CTA) : l'arm constellation (défaut) — le fond du modal coule golden-hour (ciel #0B2230 → mer #08251F → nuit #0A1714) au lieu d'une feuille plate sombre → la premium est UNE scène continue (constellation+Veilleur → promesse/preuve → plan → CTA), plus un hero golden-hour suivi d'un mur sombre. Holdout 15%. build+smoke+screenshot OK. SW v170.
- **Pulse à chaque cycle** : `node scripts/automation/pulse.cjs` → pulse.json. #1 = modal→CTA 0%/2.3% (conversion). Paywall fortement refondu (golden-hour partout) → en mesure (deploy v167→v170 propage), re-mesurer J+1 avant d'itérer (anti over-testing).
- **Carte-Fog + streak de veille** (`f438c6a6`, fallback pulse : paywall sans signal) : levier RÉTENTION « payer = habitude » (brief #5, Zenly). Streak de jours d'ouverture (`sg_veille_*`) badge calme « 🔥 N · X/Y » + fog subtil (.66 + anneau pointillé) sur plages non-consultées (`sg_consulted`, first-party) — statut JAMAIS voilé, Archipel reste vif, ouvrir révèle. `?fog=0` kill. Vérifié build+smoke+DOM(52 anneaux+badge,0err)+screenshot. SW v171.
- **PROCHAIN (design+conversion, continu, ne PAS conclure fini)** : Leaflet-out (sign-off GO ; recâbler pin-switch/suggestions d'abord) ; écrans transitionnels (onboarding, prélude, retour post-checkout) au niveau home ; handoff forecast = disclaimer UI « alertes ~24h après l'obs satellite » + tisser `weekly[id].regimeConfidence` (preuve honnête par régime) dans fiche/premium ; BeachScene inc 3-7 (semblent faits — re-vérifier à l'écran). Trip Pass : code prêt, le fondateur lance le lien Stripe.
