...
**Merge → deploy → vérif (AUTOMATIQUE, ne JAMAIS demander)** : toute correction/amélioration/fix qui passe le Gate de ship part **directement** en prod — crée une branche, ouvre une PR via github MCP, **merge sur `main` sans demander confirmation**. Ne JAMAIS clore une correction par « veux-tu que je merge et déploie ? » : la réponse est **toujours oui**, c'est le défaut (mandat fondateur 100 % mobile/absent). Tu merges, tu déploies, **puis** tu rends compte. Push sur `main` déclenche `daily-copernicus.yml` (`event_name == 'push'` → build 5 régions + deploy FTP + health-check, plafond `timeout-minutes: 75`, AUCUNE étape fondateur). Vérif DONE : **run du workflow vert** (`mcp__github__actions_list/get`) **ET** `curl` sur l'URL de prod — jamais « live » sans l'avoir vu sur prod. (Seules exceptions = le bloc « SEULS vrais blocages » ci-dessous ; tout le reste se merge et se déploie sans question.)

**SEULS vrais blocages (escalade, et SEULEMENT eux)** :
- **Apps Script** (`scripts/appscript/Code.js`) : toute modif ⇒ `clasp push` ⇒ ordinateur ⇒ fondateur mobile ⇒ **BLOQUÉ**. **Ne crée JAMAIS de nouvelle action `Code.js`.** Réutilise l'existant (ex. `POST {type:'email_signup', email, source, island}` → onglet `emails`, zéro push). Tout nouvel état serveur → **Supabase**. Dette referral en attente → ne pas multiplier. **Le compteur funnel `sg_pass_cta` a été MIGRÉ vers Supabase (table `analytics_events` + `scripts/automation/funnel-from-supabase.cjs`, 2026-07-01) → il ne nécessite PLUS de `clasp push`.**
- **Creds tiers côté console** = catégorie de blocage générique (action fondateur). **⚠️ Les items historiques de cette ligne sont RÉSOLUS (2026-07-01), ne plus les traiter comme bloqués** : GSC US provisionnée (domaines validés par le fondateur + service account vérifié + sitemaps soumis, run `provision-gsc.yml` vert) ; IDs GA4/Clarity US déjà dans `regions/*.json` (florida/puntacana/rivieramaya) ; lien Pro 690 € **validé par un vrai paiement test fondateur**. *(Les plans B2B récurrents ne sont PAS une action fondateur : montants en repo via `mol_b2b_plans`, #210.)*
...
- **Tokens comic INERTES** (Themes.css, marqueur `DETTE-TOKENS-INERTES`) : `var(--sg-*)` sous `.theme-comic` résout les tokens LIGHT d'index.html:96 (#FFFFFF/#000000), jamais la palette comic — pour du papier/encre, valeurs en dur (cf. app-runtime.css ~230). Corollaire : `#root` est EFFONDRÉ (~0px) sous comic en navigateur (marqueur `MINE-ROOT-RELATIVE`) → tout écran plein-viewport = `position:fixed` OU portal, jamais `absolute`.
...
- **Merge + deploy AUTOMATIQUES, zéro confirmation** — dès qu'une correction ou une amélioration passe le Gate de ship, tu merges sur `main` et tu déploies **sans demander**. Ne jamais terminer une tâche par « est-ce que je merge et déploie ? » : c'est le défaut implicite (fondateur 100 % mobile/absent, mandat « agis puis rends compte »). Tu livres, **ensuite** tu signales que c'est en prod (run vert + `curl`). Demander la permission de merger = anti-pattern interdit.
