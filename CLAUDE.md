...
- **B2B annuel (paylinks Mollie LIVE)** : `brief_annual` = 290 € · `pro_annual`. **⚠️ DÉCALAGE PRIX ACTIF** : décision panel 2026-06-29 = Pro annuel **690 €** (`mollie-paylinks.cjs` TIERS), MAIS le paylink déployé (`public/api/b2b-paylinks.json`, `pro_annual`) encaisse encore **790 €** (le script ne régénère pas un `pl_` déjà créé). **Pour appliquer 690 : supprimer/recréer le lien Mollie** — le simple re-run du pipeline ne corrige PAS. Tant que non fait, le live = 790. Lever ce décalage est une tâche argent prioritaire.
- **B2B mensuel récurrent (79 / 29 €) = PAS encore live** : plans Mollie récurrents à injecter dans `mollie-config.php` (hors repo) = action fondateur. **Ne vends/câble PAS de récurrent tant que les plans n'existent pas.** Idem abos récurrents B2C.
- **Essai Pro** : 21 j sans carte, 100 % self-serve (confirmé).
- **Source unique pricing** = `scripts/automation/mollie-paylinks.cjs` (TIERS). Tout changement : éditer TIERS → grep les 4 `B2B_*.md` → **régénérer le lien** (`node scripts/automation/mollie-paylinks.cjs`, `--dry` pour preview ; tourne déjà dans `daily-copernicus.yml:190`). Pour changer un prix d'un lien EXISTANT : supprimer l'ancien `pl_` dans `b2b-paylinks.json` AVANT le run, sinon il est conservé tel quel.
...
**Check-list avant merge money-path** : (1) **additif only** ; (2) **`php -l`** sur chaque `.php` touché ; (3) **revue panel adverse** (outil Workflow, voir mandat) — tu ne testes pas l'API live en local ; (4) **1 vrai paiement test après deploy** = action fondateur mobile (vérité = dashboard Mollie ; il a les creds, toi non) — tu prépares + signales, il valide ; (5) **L'argent ne touche JAMAIS le verdict** (encart Partenaire = `sponsored` ; verdict 100 % data ERDDAP). Si le paiement test échoue : revert additif (le commit précédent reste déployable), ne « répare » pas à l'aveugle.
...
- **Creds tiers côté console** : sitemaps GSC + IDs GA4/Clarity US dans `regions/*.json` (goulot SEO ×10) ; plans Mollie récurrents à injecter dans `mollie-config.php` ; paiement test réel (dashboard Mollie). = action fondateur.
...
- **Pricing arrêté (panel 2026-06-29)** : **Pro 79 €/mois ou 690 €/an** (2 mois offerts) · essai **21 j gratuit sans carte** · garantie 30 j. Brief 29 €/mo = decoy. USD : 89 $/mo · 790 $/an.
- **DEUX sources de prix B2B** (ne grep pas qu'une seule) :
  ...
  2. **Mensuel + USD** → `mol_b2b_plans()` dans `public/api/mollie.php` (plans `pro_monthly`/`brief_monthly`, montants en config gitignored). Le 79 €/mo et les tiers USD vivent ICI, pas dans le cjs.
- **⚠️ Changer un prix ANNUEL n'est PAS un simple edit du `value`** : `mollie-paylinks.cjs` fait `continue` si `out.links[tier.id].url` existe → un lien déjà créé n'est JAMAIS recréé sur changement de montant. **Procédure** : supprimer l'entrée du tier dans `public/api/b2b-paylinks.json` (ou nouvel `id`) AVANT le re-run, sinon **no-op silencieux**. Puis grep les `B2B_*.md`. **État live à corriger : `b2b-paylinks.json` porte encore `pro_annual` 790 € — re-générer pour facturer 690.**
...
- **Supabase (photos visiteurs)** : `src/supabasePhotos.js` (REST + Storage, clé anon RLS) + `src/WebcamPanel.jsx` ; Edge Function `moderate` + `notify-photos.yml`, modération 1-tap par email (secrets `SUPABASE_SERVICE_KEY` / `MODERATE_TOKEN`). **Tout NOUVEL état serveur → Supabase, jamais Apps Script.**
...
- **Modèle** : B2C **pass one-time** (live EUR MQ/GP + USD florida/puntacana/rivieramaya) **+ abos Mollie en chantier** (récurrent B2C/B2B PAS encore live, plans à créer = fondateur). Caisse = **Mollie on-site** (Components + Apple/Google Pay) ; **Stripe = legacy lecture seule** ; **PayPal** = secondaire vivant.
...
- **B2B** : Pro 79 €/mo ou 690 €/an (panel 2026-06-29), essai 21 j sans carte. **⚠️ paylink live encore à 790 € — re-générer pour 690** (procédure dans Doctrine Growth).
