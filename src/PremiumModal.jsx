      ...
      <div ref={dlgRef} role="dialog" aria-modal="true" aria-label={_t(lang,"Offre Pro — Hôtels & collectivités","Pro offer — Hotels & towns","Oferta Pro — Hoteles y municipios")} onClick={e=>e.stopPropagation()}
        ...
        {!sent&&!seqOn?<>
          ...
          <div style={{padding:"11px 12px",border:`2.5px solid ${I.ink}`,borderRadius:14,background:"#fff",boxShadow:`2px 2px 0 ${I.ink}`,marginBottom:12}}>
            ...
            <a href={_relHref(lang)} target="_blank" rel="noopener" onClick={()=>{try{track("sg_b2b_rel_click",{step:2})}catch(_){}}} style={{display:"inline-block",font:"800 12px/1.4 'Bricolage Grotesque'",color:I.ink,textDecoration:"underline",marginTop:6}}>{_t(lang,"Vérifier le registre public →","Check the public record →","Ver el registro público →")}</a>
          </div>
    ...
    <div className="pwx-wrap">
      ...
      <div className="pwx-body">
        <h2 className="pwx-title">{calm
          ...
          :ST==="avoid"?_t(lang,<>Évite les plages <em>chargées</em><br/>pendant ton séjour</>,<>Avoid <em>sargassum-heavy</em><br/>beaches on your trip</>,<>Evita playas <em>con sargazo</em><br/>durante tu estancia</>)
          :ST==="moderate"?_t(lang,<>Surveille ta plage <em>avant</em><br/>qu'elle ne bascule</>,<>Watch your beach <em>before</em><br/>it flips</>,<>Vigila tu playa <em>antes</em><br/>de que cambie</>)
        ...
        <p className="pwx-sub">{calm
          ...
          :ST==="avoid"?_t(lang,<>Cette plage est <b>saturée</b> aujourd'hui. Le Veilleur te prévient quand une <b>plage voisine redevient propre</b>.</>,<>This beach is <b>heavy</b> today. The Watcher alerts you when a <b>nearby beach clears up</b>.</>,<>Esta playa está <b>cargada</b> hoy. El Vigía te avisa cuando una <b>playa cercana mejora</b>.</>)
          :ST==="moderate"?_t(lang,<>Le gratuit te dit aujourd'hui. Le Veilleur te dit <b>demain</b> — et t'alerte le jour où ça bascule.</>,<>Free tells you today. The Watcher tells you <b>tomorrow</b> — and alerts you the day it flips.</>,<>Lo gratis te dice hoy. El Vigía te dice <b>mañana</b> — y te avisa el día que cambia.</>)
        ...
        {recordProof
          ?<div className="pwx-record"><span className="pwx-rdot">●</span><span><b>{recordProof}</b><em>{_t(lang,"La seule carte qui publie aussi ses erreurs.","The only map that also publishes its misses.","El único mapa que también publica sus errores.")}</em>{(typeof window==="undefined"||!/[?&]pwrel=0/.test(window.location.search))&&<a href={_relHref(lang)} target="_blank" rel="noopener noreferrer" onClick={()=>{try{track("sg_reliability_open",{from:"paywall_record"})}catch(_){}}} style={{display:"inline-block",marginTop:4,fontSize:10.5,fontWeight:800,color:"var(--grn)",textDecoration:"underline"}}>{_t(lang,"Vérifier le registre →","Check the record →","Ver el registro →")}</a>}</span></div>
