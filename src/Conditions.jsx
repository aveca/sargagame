/**
 * Conditions — /conditions/<slug>/ and /conditions/ (A/B `pw_conditions`)
 * Display beach selections based on weather and sargassum conditions.
 */
import React,{useRef,useEffect,useState} from "react"
import {conditionPages} from "./lib/conditions-filters.js"
...
function buildCard(b, i, lang, geo, onOpen, weather) {
  var best = i === 0;
  ...
  thumb.appendChild(svgNode(thumbSVG(b._seed || (b.id.charCodeAt(0) || 1))));
  
  var statusClass = b.status === "avoid" ? "badge avoid" : (b.status === "moderate" ? "badge mod" : "badge");
  thumb.appendChild(el("span", statusClass, statusLabels[b.status] || statusLabels.clean));
  
  if (best) thumb.appendChild(el("span", "ribbon", _t(lang, "le + sûr", "safest", "más segura")));
  card.appendChild(thumb);
  
  var body = el("div", "body");
  body.appendChild(el("div", "bname", b.name));
  body.appendChild(el("div", "bplace", b.commune + (b.kids ? " - " + _t(lang, "enfants ok", "kids ok", "niños ok") : "")));
  
  var row = el("div", "brow");
  var distVal = geo && b._dist != null 
    ? _t(lang, "vers ", "to ", "hasta ") + b._dist.toFixed(0) + " km" 
    : _t(lang, "env. ", "about ", "aprox. ") + (b.drive ? b.drive + " min" : "");
  var dist = el("span", "bdist" + (geo ? "" : " nodist"), distVal);
  
  var sc = el("span", "bscore");
  sc.appendChild(el("span", "n", String(b.score != null ? b.score : "—")));
  ...
  card.addEventListener("click", function() { onOpen(b) });
  return card;
}

export function initConditions(SR, HOST, opts) {
  var cleaners = [];
  function on(el, ev, fn, o) {
    el.addEventListener(ev, fn, o);
    cleaners.push(function() { el.removeEventListener(ev, fn, o); });
  }
  var dead = false, rafId = 0;
  var LANG = opts.lang || "fr";
  var BEACHES = opts.beaches || [];
  ...
  function render() {
    var rail = SR.getElementById("clRail");
    if (!rail) return;
    while (rail.firstChild) rail.removeChild(rail.firstChild);
    
    var page = conditionPages.find(p => p.slug === SLUG);
    
    // Title & Intro
    var kicker = SR.getElementById("clKicker");
    if (kicker) kicker.textContent = page ? _t(LANG, "Conditions de baignade", "Bathing conditions", "Condiciones de baño") : _t(LANG, "Toutes les conditions", "All conditions", "Todas las condiciones");
    ...
    var subText = SR.querySelector(".pb-sub");
    if (subText && page) {
      // Append intro under title
      var introPara = SR.getElementById("clIntroPara");
      if (!introPara) {
        introPara = el("p", "intro-p", page.intro);
        introPara.id = "clIntroPara";
        introPara.style.color = "rgba(255,255,255,.72)";
        introPara.style.fontSize = "13px";
        introPara.style.margin = "4px 0 10px";
        introPara.style.lineHeight = "1.4";
        subText.parentNode.insertBefore(introPara, subText.nextSibling);
      } else {
        introPara.textContent = page.intro;
      }
    }
    ...
    var matching = [];
    if (page) {
      matching = BEACHES.filter(b => page.filter(b, WEATHER[b.id]));
    }
    
    if (matching.length > 0) {
      matching.forEach(function(b, i) {
        rail.appendChild(buildCard(b, i, LANG, GEO, H.onOpenBeach, WEATHER[b.id]));
      });
    } else {
      var emptyMsg = el("div", "empty-msg", page ? page.fallback : _t(LANG, "Sélectionnez un filtre dans le menu", "Select a filter in the menu", "Seleccione un filtro en el menú"));
  ...
  function loop(ts) {
    ...
    var t = ts / 1000;
    vBody.setAttribute("transform", "scale(" + (1 + Math.sin(t * 0.7) * 0.012) + ")");
    beam.setAttribute("transform", "rotate(" + (Math.sin(t * 0.28) * 10) + ")");
  ...
  return {
    teardown: function() {
      ...
      for (var i = 0; i < cleaners.length; i++) {
        try { cleaners[i](); } catch (e) {}
      }
    },
    update: function(n) {
      if (dead) return;
      if (n.lang) LANG = n.lang;
      if (n.beaches) BEACHES = n.beaches;
      if (n.weather) WEATHER = n.weather;
...
export default function Conditions(props) {
  const {lang,sargData,allBeaches,beachesWeather,userPos,onOpenBeach,onShowMap,onPremium,track}=props;
  ...
  useEffect(() => {
    ...
    styleEl.textContent = COND_CSS;
    SR.appendChild(styleEl);
    
    SR.appendChild(document.createRange().createContextualFragment(SCENE_MARKUP));
    
    var planb = document.createElement("section");
    planb.id = "planb";
    planb.innerHTML = [
      '<div class="pb-head">',
      '  <div>',
      '    <div class="pb-kicker" style="display:flex;align-items:center;gap:10px">',
      '      <button id="clBackBtn" class="back-btn">← Home</button>',
      '      <span id="clKicker"></span>',
      '    </div>',
      '    <h2 class="pb-title anton" id="clTitle"></h2>',
      '    <div class="pb-sub"><span class="pb-live"><b></b><span id="clLiveTxt"></span></span></div>',
      '  </div>',
      '</div>',
      '<div class="pb-rail" id="clRail"></div>'
    ].join("");
    SR.appendChild(planb);
    
    const backBtn = SR.getElementById("clBackBtn");
    if (backBtn) {
      backBtn.addEventListener("click", () => {
        try {
          cbRef.current.onShowMap && cbRef.current.onShowMap();
        } catch (_) {}
      });
    }
    ...
    const hooks = {
      track: (n, p) => {
        try { cbRef.current.track && cbRef.current.track(n, p); } catch (_) {}
      },
      onOpenBeach: (b) => {
        try { cbRef.current.onOpenBeach && cbRef.current.onOpenBeach(b); } catch (_) {}
      },
      onShowMap: () => {
        try { cbRef.current.onShowMap && cbRef.current.onShowMap(); } catch (_) {}
      }
    };

    let eng = null;
    try {
      const island = allBeaches && allBeaches[0] ? allBeaches[0].island : "mq";
      eng = initConditions(SR, host, {
        lang: lang || "fr",
        beaches: allBeaches || [],
      ...
      console.error("Conditions init failed:", e);
    }
    engRef.current = eng;

    // Track view of this condition page
    try {
      cbRef.current.track && cbRef.current.track("sg_conditions_view", { slug });
    } catch (_) {}

    return () => {
      try { eng && eng.teardown(); } catch (_) {}
      engRef.current = null;
      try { while (SR.firstChild) SR.removeChild(SR.firstChild); } catch (_) {}
    };
  }, [slug]);

  useEffect(() => {
    if (engRef.current) {
      engRef.current.update({
        lang: lang || "fr",
        beaches: allBeaches || [],
        weather: beachesWeather || {},
        updatedAt: sargData && (sargData.updatedAt || sargData.erddapTimestamp),
  ...
  return React.createElement("div", {
    ref: hostRef,
    role: "dialog",
    style: {position:"absolute",inset:0,zIndex:1050,overflow:"hidden",background:"#0a1620"}
  });
}
