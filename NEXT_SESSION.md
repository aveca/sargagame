...
> **🎯 2026-09-16 — P0 MONEY-PATH (PR #682) : garde mounts + taxonomy + tag synthetic — CI en cours, puis merge → deploy → vérif prod sans submit**
> Cause : tokenize sans mounts (7× `sg_payment_failed` prod, texte vendeur brut). Fix : `payMountedRef` partagé + attente 6 s + retry + friendly + `sg_mollie_components_not_mounted`. Suites E2E innocentées (stub + consent refusé). Preuves locales vertes (build/bundle/smoke/contrat 12/12/T1/funnel 13/13/fast-click → tokenize atteint).
