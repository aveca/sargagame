...
- [x] 14 CLASSEMENT DES VEILLEURS (leaderboard) — modale `LadderSheet` ouverte depuis l'en-tête collection (bouton 🪜 à côté de Badges). Un « top joueurs » serveur est IMPOSSIBLE (app mono-joueur 100% local `sg_chasse`, zéro roster) → inventer des adversaires = fabrication interdite. Réponse HONNÊTE = échelle RÉELLE des 6 rangs `TIERS` (Promeneur→Légende du Lagon) que le joueur gravit déjà : rang courant dérivé de `collSet.size`, paliers atteints ✓ / verrouillés 🔒, tag « TOI », distance au rang suivant, stats réelles (collection + record de série `st.best`). 0 fabrication, i18n fr/en/es, a11y (role=dialog, Escape, focus), reduced-motion plancher, réversible `?ladder=0` (force-open `?ladder=1`). Vérifié WebKit mobile (6 tiers, TOI sur Éclaireur à 13 collectées, foot « encore 12 plages », rollback masque le bouton, ux-smoke ERRORS=[]). → 13
