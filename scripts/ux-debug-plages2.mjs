// Quick debug: check beach data in preview with detailed logging
...
p.on('request', (r) => { if (r.url().includes('sargassum') || r.url().includes('forecast')) console.log('REQ ' + r.url().slice(-60)); });
p.on('response', (res) => { if (res.url().includes('sargassum') || res.url().includes('forecast')) console.log('RES ' + res.status() + ' ' + res.url().slice(-60)); });
const p = await ctx.newPage();
