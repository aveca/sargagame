...
function termsContent(lang, region) {
  ...
  if (lang === 'es') {
    return {
      ...
      sections: [
        ...
        { h: '3. Ofertas y precios', html: `<p>Las ofertas y precios vigentes se presentan en la aplicación. A día de hoy:</p><ul><li><strong>Premium mensual</strong> — ${esc(p.monthly || '')} /mes, sin compromiso.</li><li><strong>Premium anual</strong> — ${esc(p.yearly || '')} /año.</li><li><strong>Pase de viaje</strong> — ${esc(p.tripPass || '')}, acceso completo 7 días, pago único sin renovación.</li></ul><p>Los precios se indican en ${esc(cur)}, impuestos incluidos cuando correspondan; los impuestos aplicables los calcula y recauda nuestro proveedor de pago según tu lugar de residencia.</p>` },
        { h: '4. Pago', html: `<p>${paymentClause(lang)}</p>` },
        { h: '5. Suscripción y renovación', html: `<p>Las suscripciones Premium (mensual, anual) se renuevan por <strong>tácita reconducción</strong> al vencimiento, a la tarifa vigente, hasta su cancelación. El <strong>Pase de viaje</strong> es una compra única, no renovable.</p>` },
        { h: '6. Cancelación', html: `<p>Puedes cancelar tu suscripción <strong>en cualquier momento</strong>, sin gastos ni justificación, desde el espacio de gestión de tu suscripción o escribiéndonos a <a href="mailto:${esc(support)}">${esc(support)}</a>. La cancelación surte efecto al final del periodo ya pagado; no se realiza ningún cobro posterior y el acceso se mantiene hasta esa fecha.</p>` },
  ...
  return {
    ...
    sections: [
      ...
      { h: '3. Offers and prices', html: `<p>Current offers and prices are shown in the app. As of today:</p><ul><li><strong>Premium monthly</strong> — ${esc(p.monthly || '')}/month, no commitment.</li><li><strong>Premium yearly</strong> — ${esc(p.yearly || '')}/year.</li><li><strong>Trip Pass</strong> — ${esc(p.tripPass || '')}, full access for 7 days, one-time payment with no renewal.</li></ul><p>Prices are shown in ${esc(cur)}, including applicable taxes where due; applicable taxes are calculated and collected by our payment provider based on your place of residence.</p>` },
      { h: '4. Payment', html: `<p>${paymentClause(lang)}</p>` },
      { h: '5. Subscription and renewal', html: `<p>Premium subscriptions (monthly, yearly) <strong>renew automatically</strong> at term, at the current rate, until cancelled. The <strong>Trip Pass</strong> is a one-time purchase, non-renewing.</p>` },
      { h: '6. Cancellation', html: `<p>You can cancel your subscription <strong>at any time</strong>, with no fee or justification, from your subscription management area or by writing to <a href="mailto:${esc(support)}">${esc(support)}</a>. Cancellation takes effect at the end of the period already paid; no further charge is made and access remains active until then.</p>` },
...
function refundContent(lang, region) {
  ...
  if (lang === 'es') {
    return {
      ...
      sections: [
        { h: 'Garantía de satisfacción — 30 días', html: `<div class="card"><p style="margin:0">Si el Servicio no te convence, puedes solicitar un <strong>reembolso íntegro en un plazo de 30 días</strong> tras tu primer pago (suscripción mensual o anual). No hace falta justificación. Es nuestro compromiso «satisfecho o reembolsado».</p></div>` },
        ...
        { h: 'Cancelación vs reembolso', html: `<p>La <strong>cancelación</strong> detiene los cobros futuros (el acceso sigue activo hasta el final del periodo ya pagado) — no es un reembolso. El <strong>reembolso</strong> te devuelve el importe pagado según las condiciones anteriores.</p>` },
  ...
  return {
    title: 'Refund Policy',
    lead: `This page describes the refund policy for the Premium offers of <strong>${esc(site)}</strong> (${esc(region.domain)}).`,
    sections: [
      { h: '30-day money-back guarantee', html: `<div class="card"><p style="margin:0">If the Service isn’t right for you, you can request a <strong>full refund within 30 days</strong> of your first payment (monthly or yearly subscription). No justification needed. That’s our “satisfied or refunded” commitment.</p></div>` },
      ...
      { h: 'Cancellation vs refund', html: `<p><strong>Cancellation</strong> stops future charges (access stays active until the end of the period already paid) — it is not a refund. A <strong>refund</strong> returns the amount paid under the conditions above.</p>` },
