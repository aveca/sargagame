...
// Retire de dist/sitemap*.xml toute entrée <url> dont le <loc> ne pointe PAS
// vers `domain` (la région du build courant). Constat LIVE 2026-09-10 : la boucle
// Sprint #25 (toutes régions dans chaque build, cross-domain voulu pour les PAGES)
// laissait aussi les locs étrangères dans chaque sitemap.xml (ex. MQ : 222 MQ +
// 91 GP + 27 Miami + 20 PC + 27 Cancun + 16 Tulum). Google ignore les locs
// étrangères (budget crawl gaspillé + bruit GSC). Les PAGES multi-domaines restent
// sur disque (cross-domain honnête + canonicals corrects, inchangés) — seul le
// sitemap est filtré. Idempotent. Rollback : revert ce commit.
function pruneForeignDomains(distDir, domain) {
  const res = { files: 0, removed: 0, kept: 0, domain: String(domain || '').toLowerCase() };
  if (!res.domain) return res;
  let files = [];
  try { files = fs.readdirSync(distDir).filter((f) => f === 'sitemap.xml' || (f.startsWith('sitemap-') && f.endsWith('.xml'))); }
  catch (_) { return res; }
  for (const f of files) {
    ...
    for (let i = 1; i < parts.length; i += 2) {
      ...
      let host = '';
      try { host = new URL(loc.trim()).hostname.toLowerCase(); } catch (_) {}
      if (host && host !== res.domain) { removed++; keptHead += tail; }
      else { kept++; keptHead += entry + tail; }
    }
    res.removed += removed;
    res.kept += kept;
...
module.exports = { pruneDeadDomains, deadDomains, pruneForeignDomains };
