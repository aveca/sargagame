 ...
 * tue le partage natif). Image 1080×1350 (4:5, portée optimale FB/IG feed).
 *
...
function build(profile) {
  ...
  // 2 demi-cartes en bas
  const halfW = (cardW - 32) / 2

  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}" viewBox="0 0 ${W} ${H}">
  ${L.commonDefs()}
  <rect width="${W}" height="${H}" fill="url(#bg)"/>
  <circle cx="${W - 40}" cy="120" r="240" fill="${L.PALETTE.teal}" opacity="0.07"/>
  <circle cx="120" cy="${H - 120}" r="200" fill="${L.PALETTE.gold}" opacity="0.05"/>
  <rect width="${W}" height="${H}" fill="url(#vign)"/>

  <!-- En-tête -->
  ${L.wordmark(PAD, 120, 'SARGASSES', { size: 26 })}
  <text x="${W - PAD}" y="120" text-anchor="end" font-family="${L.FONT}" font-size="24" font-weight="700" letter-spacing="3" fill="${L.PALETTE.mut}">${L.esc(reg.label)}</text>

  <!-- Titre -->
  <text x="${PAD}" y="${titleY}" font-family="${L.FONT}" font-size="96" font-weight="900" fill="${L.PALETTE.white}" letter-spacing="-2">MON ÉTÉ</text>
  <text x="${PAD}" y="${titleY + 104}" font-family="${L.FONT}" font-size="96" font-weight="900" fill="url(#goldgrad)" letter-spacing="-2">SARGASSES</text>
  <text x="${PAD}" y="${titleY + 158}" font-family="${L.FONT}" font-size="30" font-weight="700" letter-spacing="6" fill="${L.PALETTE.teal}">${L.esc(profile.season.toUpperCase())}</text>

  <!-- Stat héro : jours vérifiés -->
  ${bigStatCard(cardX, 500, cardW, 200, String(profile.daysChecked), 'jours', "à checker ma plage avant de partir", L.PALETTE.gold)}

  <!-- Plage préférée -->
  <rect x="${cardX}" y="724" width="${cardW}" height="208" rx="30" fill="${L.PALETTE.card}" stroke="${L.PALETTE.cardLine}"/>
  <text x="${cardX + 42}" y="${724 + 56}" font-family="${L.FONT}" font-size="25" font-weight="700" letter-spacing="3" fill="${L.PALETTE.teal}">MA PLAGE DE L'ÉTÉ</text>
  <text x="${cardX + 40}" y="${724 + 128}" font-family="${L.FONT}" font-size="64" font-weight="900" fill="${L.PALETTE.white}">${L.esc(favName)}</text>
  ${favScore != null
      ? `<text x="${cardX + 42}" y="${724 + 178}" font-family="${L.FONT}" font-size="27" font-weight="600" fill="${L.PALETTE.mut}">${favScore}/100 aujourd'hui</text>
         <circle cx="${cardX + cardW - 92}" cy="${724 + 104}" r="58" fill="none" stroke="${(L.STATUS[favStatus] || L.STATUS.clean).color}" stroke-width="8" opacity="0.9"/>
         <text x="${cardX + cardW - 92}" y="${724 + 120}" text-anchor="middle" font-family="${L.FONT}" font-size="48" font-weight="900" fill="${L.PALETTE.white}">${favScore}</text>`
      : ''}

  <!-- 2 demi-cartes -->
  ${bigStatCard(cardX, 956, halfW, 188, String(profile.cleanDaysFound), 'j', 'plage propre trouvée', L.PALETTE.teal)}
  ${bigStatCard(cardX + halfW + 32, 956, halfW, 188, String(profile.alertsAvoided), '', 'alertes esquivées', '#6AC15A')}
...
async function run() {
  ...
  const { svg, region } = build(profile)
  const date = L.todayISO()
  const tag = profile._demo ? 'demo' : (profile.userId || 'user')
  const out = path.join(L.OUT_DIR, `wrapped-${region}-${tag}-${date}.png`)
  const res = await L.renderSVG(svg, out)
  console.log(`✓ Beach Wrapped ${region} → ${path.relative(L.ROOT, res.path)}`)
